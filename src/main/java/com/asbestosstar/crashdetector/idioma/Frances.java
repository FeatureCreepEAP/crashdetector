...
public class Frances implements Idioma {
	...
	public String anonimizarRegistros() {
		return "Anonymiser les journaux";
	}
...
//En tu clase de idioma:
	...
	public String migradorCAModoPrincipalAviso() {
		return "Choisissez comment migrer l'interface principale. \"centro_soporte\" est le plus proche de l'interface par défaut de Crash Assistant, mais n'est pas recommandé, surtout pour les joueurs DIY, car il souffre d'un enfer de popups et d'un design médiocre ; il existe uniquement pour aider à migrer les environnements legacy de Crash Assistant. \"primitiva\" est une interface très simple utilisée uniquement pour le partage ; elle n'est suggérée que pour les modpacks, pas pour les utilisateurs DIY, et même si vous désactivez toutes les vérifications inutiles, elle souffre aussi d'un enfer de popups. \"original\" conserve votre interface principale actuelle et est généralement l'option recommandée pour la plupart des utilisateurs.";
	}

	@Override
	public String migradorCASitioLoggingAviso() {
		return "Choisissez si vous souhaitez migrer le site de journalisation depuis Crash Assistant ou conserver celui actuel de CrashDetector. Crash Assistant utilise souvent gnomebot.dev, qui n'est pas recommandé par rapport à SecureLogger, PastesDev ou CDPaste. Pour la plupart des utilisateurs, il est recommandé de conserver le site actuel.";
	}

	// French (Burkina Faso variant) (Français)
	@Override
	public String ideScriptTitulo() {
		return "IDE de script";
	}
	...
	public String ideScriptProyectoDeshabilitadoAviso() {
		return "Ce type de projet existe déjà dans l'interface, mais il est désactivé jusqu'à ce que son serveur de langage ou son analyseur soit ajouté.";
	}
	...
	public String ideScriptConfirmarDescargaDeps(int cantidad, String lista) {
		return cantidad + " dépendances seront téléchargées pour l'IDE de scripts :\n\n" + lista
				+ "\nIl peut être nécessaire de redémarrer ensuite pour qu'elles soient prises en compte dans le classpath.";
	}

	@Override
	public String ideScriptDepsDescargadas(String mensaje) {
		return "Résultat du téléchargement :\n\n" + mensaje
				+ "\n\nRedémarrez le programme si les classes n'apparaissent pas encore dans le classpath.";
	}
	...
	public String ideScriptColorEditor() {
		return "Couleur de l'éditeur";
	}

	@Override
	public String ideScriptColorKeyword() {
		return "Couleur des mots-clés";
	}
	...
	public String mcpTituloPrincipal() {
		return "Serveur MCP pour IA locale";
	}
	...
	public String mcpDescargarDependencias() {
		return "Télécharger les dépendances MCP/CFR";
	}

	public String mcpIniciarServidor() {
		return "Démarrer le serveur MCP";
	}

	public String mcpEstadoDependenciasNoCargadas() {
		return "Dépendances MCP/CFR non chargées. Téléchargez-les et redémarrez si le bouton reste désactivé.";
	}

	public String mcpEstadoDependenciasCargadas() {
		return "Dépendances MCP/CFR détectées. Le serveur MCP peut être démarré.";
	}

	public String mcpDependenciasDescargadasReiniciar() {
		return "Dépendances téléchargées. Si elles n'apparaissent pas encore chargées, redémarrez CrashDetector.";
	}

	public String mcpErrorDescargandoDependencias(String error) {
		return "Impossible de télécharger les dépendances MCP/CFR : " + error;
	}

	public String mcpServidorIniciado(int puerto) {
		return "Serveur MCP démarré sur 127.0.0.1:" + puerto + "/mcp";
	}

	public String mcpErrorIniciandoServidor(String error) {
		return "Impossible de démarrer le serveur MCP : " + error;
	}

	public String mcpImagenNoDisponible() {
		return "Image MCP non disponible";
	}

	public String colorAcento() {
		return "Couleur d'accentuation";
	}

	public String mcpDescripcionHtml() {
		return "<b>Ce panneau démarre un serveur MCP local pour des assistants comme Claude Desktop, Qwen Desktop Linux, Red Hat command-line-assistant ou Goose.</b>"
				+ "<br><br>"
				+ "Téléchargez d'abord les dépendances MCP/CFR. Redémarrez ensuite si les classes n'apparaissent pas dans le classpath."
				+ "<br><br>" + "<b>Claude Desktop / Qwen Desktop Linux, exemple :</b>" + "<pre>{\n"
				+ "  \"mcpServers\": {\n" + "    \"crashdetector\": {\n"
				+ "      \"url\": \"http://127.0.0.1:8765/mcp\"\n" + "    }\n" + "  }\n" + "}</pre>"
				+ "<b>Goose / command-line-assistant :</b>" + "<pre>http://127.0.0.1:8765/mcp</pre>"
				+ "Pour l'instant, ce serveur MCP est basique. Par la suite, on pourra ajouter de vrais outils pour lire les journaux, consulter CFR, vérifier les mods, chercher des classes et expliquer les crashes.";
	}
	...
	public String mensajeErrorJvmDllCurseForgeG1(boolean conOverwolf) {
		...
		if (conOverwolf) {
			textoOverwolf = "<p>Le journal montre également des indices de <b>Overwolf</b> ou de DLL associées, telles que "
					+ "<b>OWClient.dll</b> ou <b>OWUtils.dll</b>. Cela ne prouve pas à lui seul qu'Overwolf est la cause, "
					+ "mais indique que le processus Minecraft s'exécutait dans un environnement plus modifié "
					+ "qu'un lancement vanilla normal.</p>";
		}

		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Java s'est fermé suite à une erreur native dans jvm.dll pendant le ramasse-miettes G1 sur CurseForge.</b>"
				+ "<p>Le journal contient une <b>EXCEPTION_ACCESS_VIOLATION</b> au sein de <b>jvm.dll</b>, "
				+ "et le thread actif est un <b>GCTaskThread</b>. Cela signifie que Java a échoué pendant que le ramasse-miettes "
				+ "<b>G1</b> travaillait.</p>"
				+ "<p>Il y a aussi des indices que le jeu a été lancé depuis un environnement géré par "
				+ "<b>CurseForge</b>, par exemple des chemins CurseForge, <b>DCFInstanceId</b> ou la marque du lanceur CurseForge.</p>"
				+ textoOverwolf
				+ "<p>On ne peut pas prouver uniquement à partir du journal que CurseForge ou Overwolf sont la cause exacte. "
				+ "Cependant, ce schéma est souvent vu dans les lancements CurseForge et peut être lié "
				+ "à la combinaison de <b>G1 GC</b>, mémoire native, bibliothèques Minecraft, pilotes graphiques, "
				+ "dossiers natifs gérés par CurseForge, superpositions ou composants externes du lanceur.</p>"
				+ "<p><b>Solution courante :</b></p>" + "<ul>"
				+ "<li>Changez le ramasse-miettes en ajoutant cet argument JVM : <b>-XX:+UseShenandoahGC</b></li>"
				+ "<li>Dans CurseForge, cela s'ajoute généralement dans les arguments Java supplémentaires du profil ou du lanceur.</li>"
				+ "<li>Effacez le cache natives/bin du profil CurseForge pour forcer l'extraction à nouveau des bibliothèques natives.</li>"
				+ "<li>Mettez à jour Java et les pilotes graphiques.</li>"
				+ "<li>Si vous utilisez des superpositions, enregistreurs, antivirus agressifs ou autres outils s'injectant dans les jeux, essayez de les fermer temporairement.</li>"
				+ "</ul>" + "<p>Tutoriel recommandé en anglais pour changer les arguments dans CurseForge : "
				+ "<a href='https://youtu.be/UKFWBOZxB2o'>https://youtu.be/UKFWBOZxB2o</a></p>"
				+ "<p><b>Note pour Minecraft 1.16.5 ou antérieur :</b> ces versions utilisent normalement Java 8. "
				+ "Si vous êtes sur JDK 8 et voulez utiliser Shenandoah, vous devrez peut-être utiliser le "
				+ "<b>Red Hat Build of OpenJDK 8</b> et pointer CurseForge vers cette installation Java.</p>"
				+ "<p>Guide de Red Hat pour installer OpenJDK 8 sur Windows : "
	...
	public String nombreErrorJvmDllCurseForgeG1() {
		return "Erreur jvm.dll de CurseForge avec G1 GC";
	}

	@Override
	public String mensajeErrorJvmDllC2Sodium() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Java s'est fermé suite à une erreur native dans jvm.dll lors de la compilation de code de Sodium ou d'un mod similaire.</b>"
				+ "<p>Le journal contient une <b>EXCEPTION_ACCESS_VIOLATION</b> au sein de <b>jvm.dll</b>, "
				+ "et le thread actif est un <b>C2 CompilerThread</b>. Cela signifie que l'échec s'est produit au sein du "
				+ "compilateur JIT de Java, pas comme une exception normale de Minecraft.</p>"
				+ "<p>Dans ce cas, la tâche de compilation mentionne du code lié à <b>Sodium</b>, "
				+ "<b>Embeddium</b>, <b>Rubidium</b> ou une classe similaire comme "
				+ "<b>ClonedChunkSectionCache::acquire</b>.</p>" + "<p><b>Solution recommandée :</b></p>" + "<ul>"
				+ "<li>Mettez à jour <b>Sodium</b>, <b>Embeddium</b>, <b>Rubidium</b>, <b>Oculus</b> ou tout mod de rendu associé.</li>"
				+ "<li>Mettez à jour Java 17. Si vous utilisez déjà une version moderne, essayez une autre distribution comme Temurin, Zulu ou Microsoft OpenJDK.</li>"
				+ "<li>Mettez à jour les pilotes de la carte graphique.</li>"
				+ "<li>Essayez temporairement sans Sodium, Embeddium, Rubidium ou Oculus pour confirmer si l'erreur disparaît.</li>"
				+ "<li>Comme test avancé, utilisez <b>-XX:TieredStopAtLevel=1</b> pour réduire l'utilisation du compilateur C2. "
				+ "Cela peut baisser les performances, mais aide à confirmer si l'erreur vient du compilateur JIT.</li>"
				+ "</ul>"
				+ "<p>Ce problème n'est pas le même qu'un crash normal d'un mod. Il est aussi différent des erreurs où "
				+ "<b>GCTaskThread</b> apparaît comme thread actif. Ici, le schéma pointe plus vers le compilateur C2 de Java "
				+ "compilant du code de rendu optimisé.</p>";
	}

	@Override
	public String nombreErrorJvmDllC2Sodium() {
		return "Erreur native de Java avec Sodium / Embeddium";
	}
	...
	public String mensajeErrorArchivoUsadoPorOtroProceso(String archivo) {
		...
		if (archivo != null && archivo.length() > 0) {
			textoArchivo = "<p><b>Fichier verrouillé :</b> " + archivo + "</p>";
		}

		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Un fichier est utilisé par un autre processus.</b>"
				+ "<p>Le journal contient une erreur <b>java.nio.file.FileSystemException</b> indiquant que Windows ne peut pas "
				+ "accéder à un fichier car un autre programme l'utilise.</p>" + textoArchivo
				+ "<p>Cela se produit généralement lorsque Minecraft, CurseForge, un éditeur de texte, un antivirus, OneDrive, "
				+ "un programme de synchronisation ou même une autre instance du jeu a ouvert le fichier.</p>"
				+ "<p><b>Solution recommandée :</b></p>" + "<ul>" + "<li>Fermez complètement Minecraft.</li>"
				+ "<li>Fermez CurseForge et rouvrez-le.</li>"
				+ "<li>Vérifiez le Gestionnaire des tâches et terminez les processus en double de <b>javaw.exe</b>, <b>java.exe</b>, Minecraft ou CurseForge.</li>"
				+ "<li>Fermez les éditeurs de texte qui pourraient avoir ouvert le fichier de configuration.</li>"
				+ "<li>Si le fichier se trouve dans OneDrive, Dropbox ou un programme similaire, mettez temporairement la synchronisation en pause.</li>"
				+ "<li>Si le problème persiste, redémarrez l'ordinateur pour libérer le verrouillage du fichier.</li>"
				+ "</ul>"
				+ "<p>Après avoir libéré le fichier, relancez le pack de mods. Si le fichier verrouillé est un fichier de configuration, "
				+ "il peut aussi être utile d'en faire une sauvegarde et de régénérer ce fichier.</p>";
	}

	@Override
	public String nombreErrorArchivoUsadoPorOtroProceso() {
		return "Fichier utilisé par un autre processus";
	}

	@Override
	public String nombreErrorBetterEndPaletaChunkAgua() {
		return "Erreur de palette de chunk dans BetterEnd";
	}

	@Override
	public String mensajeErrorBetterEndPaletaChunkAgua() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "BetterEnd apparaît dans un crash de rendu de chunks.</b>" + "<p>Le jeu a lancé :</p>"
				+ "<code>EntryMissingException: Missing Palette entry</code>"
				+ "<p>L'erreur se produit lors de la construction des maillages (meshes) de chunks et BetterEnd intervient "
				+ "dans la couleur de l'eau via <code>betterend$be_getWaterColor</code>.</p>"
				+ "<p>Cela pointe généralement vers un problème avec BetterEnd, BCLib/WorldWeaver, la génération du monde, "
				+ "ou une interaction avec Sodium/le rendu des fluides.</p>"
				+ "<p>Essayez de mettre à jour BetterEnd et ses dépendances, testez sans BetterEnd temporairement, "
				+ "et vérifiez si le crash ne se produit que près de certains chunks ou biomes.</p>";
	}

	@Override
	public String detectarAutomaticamente() {
		return "Détecter automatiquement";
	}

	/*
	 * Textes par défaut du scanner distant.
	 */

	public String guardAbrirEscanerNube() {
		return "VirusTotal et MetaDefender";
	}

	public String escanerNubeBotonLateral() {
		return "Scanner dans le cloud";
	}

	public String escanerNubeTitulo() {
		return "Scanner de mods avec VirusTotal et MetaDefender";
	}

	public String escanerNubeMensajeRecuerdoParallelArtistProject() {
		return "Vous nous manquez beaucoup. Votre créativité, votre énergie et tout ce que vous avez partagé avec la communauté sont toujours présents dans ce projet. Cet outil conserve avec affection le souvenir de Parallel Artist Project.";
	}

	public String escanerNubeAvisoPrivacidad() {
		return "Avis : lorsqu'un hachage n'a pas de rapport, le fichier JAR sera envoyé au fournisseur sélectionné. Les échantillons téléchargés sur des services publics peuvent être partagés avec des partenaires de sécurité. Le parallélisme n'augmente pas le quota du compte et trop de threads peuvent provoquer une erreur HTTP 429. N'envoyez pas de mods privés ou confidentiels sans vérifier les conditions de votre compte.";
	}

	public String escanerNubeClaveVirusTotal() {
		return "Clé API VirusTotal :";
	}

	public String escanerNubeClaveMetaDefender() {
		return "Clé API MetaDefender :";
	}

	public String escanerNubeNumeroHilos() {
		return "Threads simultanés :";
	}

	public String escanerNubeGuardarClaves() {
		return "Enregistrer les clés globales";
	}

	public String escanerNubeEscanearVirusTotal() {
		return "Analyser avec VirusTotal";
	}

	public String escanerNubeEscanearMetaDefender() {
		return "Analyser avec MetaDefender";
	}

	public String escanerNubeEscanearAmbos() {
		return "Analyser avec les deux";
	}
	...
	public String escanerNubeClavesGuardadas() {
		return "Les clés API globales ont été enregistrées avec succès.";
	}

	public String escanerNubeConfirmarSubidaTitulo() {
		return "Confirmer l'analyse à distance";
	}

	public String escanerNubeConfirmarSubidaMensaje() {
		return "Le hachage SHA-256 sera d'abord consulté. Les fichiers sans rapport seront téléchargés chez le fournisseur sélectionné. Souhaitez-vous continuer ?";
	}

	public String escanerNubeFaltaClaveVirusTotal() {
		return "La clé API VirusTotal est manquante.";
	}

	public String escanerNubeFaltaClaveMetaDefender() {
		return "La clé API MetaDefender est manquante.";
	}

	public String escanerNubeSinArchivosJar() {
		return "Aucun fichier JAR lisible n'a été trouvé dans les dossiers de mods configurés.";
	}

	public String escanerNubeArchivoNoLegible() {
		return "Le fichier n'existe pas ou ne peut pas être lu.";
	}

	public String escanerNubeImagenNoDisponible() {
		return "Image de Parallel Artist Project non disponible";
	}
	...
	public String escanerNubeEstadoEscaneando(int completados, int total) {
		return "Analyse en cours : " + completados + " sur " + total;
	}

	public String escanerNubeEstadoCompletado(int completados) {
		return "Analyse terminée. Tâches accomplies : " + completados;
	}
	...
	public String escanerNubeDetalleInformeExistente() {
		return "Résultat obtenu via une requête de hachage ; le fichier n'a pas été re-téléchargé.";
	}

	public String escanerNubeDetalleArchivoSubido() {
		return "Le hachage n'avait pas de rapport et le fichier a été téléchargé pour analyse.";
	}

	public String escanerNubeDetalleSinEstadisticas() {
		return "Le fournisseur a terminé la demande, mais n'a renvoyé aucune statistique utilisable.";
	}

	public String escanerNubeErrorClaveRechazada(String proveedor) {
		return proveedor + " a rejeté la clé API ou le compte n'est pas autorisé pour cette opération.";
	}

	public String escanerNubeErrorLimite(String proveedor) {
		return proveedor + " a rejeté la demande car la limite ou le contrôle de fréquence du compte a été atteint.";
	}

	public String escanerNubeErrorHttp(String proveedor, int codigo, String detalle) {
		String sufijo = detalle == null || detalle.trim().isEmpty() ? "" : " Détail : " + detalle;
		return proveedor + " a renvoyé le code HTTP " + codigo + "." + sufijo;
	}

	public String escanerNubeErrorRespuestaJson(String proveedor) {
		return proveedor + " a renvoyé une réponse JSON invalide.";
	}

	public String escanerNubeErrorTiempoEspera(String proveedor) {
		return "Le délai d'attente de l'analyse de " + proveedor + " a expiré.";
	}

	public String escanerNubeErrorIdAnalisis() {
		return "Le fournisseur n'a pas renvoyé d'identifiant d'analyse.";
	}

	public String escanerNubeErrorUrlSubidaVirusTotal() {
		return "VirusTotal n'a pas renvoyé d'URL pour télécharger le gros fichier.";
	}

	public String escanerNubeErrorUrlSubidaNoSegura() {
		return "Le fournisseur a renvoyé une URL de téléchargement qui n'utilise pas HTTPS.";
	}

	public String escanerNubeErrorDesconocido() {
		return "Une erreur inconnue s'est produite pendant l'analyse.";
	}
	...
	public String escanerNubeColorFondo() {
		return "Scanner dans le cloud : couleur de fond";
	}

	public String escanerNubeColorPanel() {
		return "Scanner dans le cloud : couleur du panneau";
	}

	public String escanerNubeColorPanelClaro() {
		return "Scanner dans le cloud : couleur du panneau clair";
	}

	public String escanerNubeColorTexto() {
		return "Scanner dans le cloud : couleur du texte";
	}

	public String escanerNubeColorTextoSecundario() {
		return "Scanner dans le cloud : couleur du texte secondaire";
	}

	public String escanerNubeColorCampo() {
		return "Scanner dans le cloud : couleur du champ";
	}

	public String escanerNubeColorCampoTexto() {
		return "Scanner dans le cloud : couleur du texte du champ";
	}

	public String escanerNubeColorVirusTotal() {
		return "Scanner dans le cloud : accent VirusTotal";
	}

	public String escanerNubeColorMetaDefender() {
		return "Scanner dans le cloud : accent MetaDefender";
	}

	public String escanerNubeColorAmbos() {
		return "Scanner dans le cloud : accent combiné";
	}

	public String escanerNubeColorAdvertencia() {
		return "Scanner dans le cloud : couleur d'avertissement";
	}

	public String escanerNubeColorTabla() {
		return "Scanner dans le cloud : couleur du tableau";
	}

	public String escanerNubeColorTablaTexto() {
		return "Scanner dans le cloud : couleur du texte du tableau";
	}

	public String escanerNubeColorSeleccion() {
		return "Scanner dans le cloud : couleur de sélection";
	}

	public String escanerNubeColorSeleccionTexto() {
		return "Scanner dans le cloud : couleur du texte sélectionné";
	}
	...
	public String gitAvanzadoTipo() {
		return "Client Git avancé";
	}

	@Override
	public String jgitAbrirClienteAvanzado() {
		return "Ouvrir le client Git avancé";
	}

	@Override
	public String gitAvanzadoTitulo() {
		return "Client Git avancé — Wendy Mizumi : Lost Media";
	}

	@Override
	public String gitAvanzadoSerieLostMedia() {
		return "Wendy Mizumi — Lost Media / archive du dépôt actuel";
	}
	...
	public String gitAvanzadoHistorial() {
		return "Historique des commits";
	}

	@Override
	public String gitAvanzadoNoPreparados() {
		return "Modifications non indexées";
	}

	@Override
	public String gitAvanzadoPreparados() {
		return "Modifications indexées";
	}

	@Override
	public String gitAvanzadoPrepararSeleccion() {
		return "Indexer la sélection";
	}
	...
	public String gitAvanzadoQuitarSeleccion() {
		return "Désindexer la sélection";
	}

	@Override
	public String gitAvanzadoQuitarTodo() {
		return "Tout désindexer";
	}
	...
	public String gitAvanzadoActualizando() {
		return "Actualisation du dépôt actuel...";
	}

	@Override
	public String gitAvanzadoJGitNoDisponible() {
		return "JGit n'est pas disponible. Installez les dépendances depuis l'interface Git normale, puis redémarrez l'application.";
	}

	@Override
	public String gitAvanzadoRepositorioRequerido() {
		return "Le client avancé ne peut être ouvert qu'après la création du dépôt de l'instance actuelle depuis l'interface Git normale.";
	}

	@Override
	public String gitAvanzadoEstadoResumen(int noPreparados, int preparados, int commits) {
		return "Non indexées : " + noPreparados + " | Indexées : " + preparados + " | Commits affichés : " + commits;
	}

	@Override
	public String gitAvanzadoMensajeRequerido() {
		return "Saisissez un message de commit.";
	}

	@Override
	public String gitAvanzadoNombreNuevaRama() {
		return "Nom de la nouvelle branche :";
	}

	@Override
	public String gitAvanzadoConfirmarCambioRama(String rama) {
		return "Passer à la branche « " + rama
				+ " » ? Les modifications locales doivent être compatibles avec ce changement de branche.";
	}

	@Override
	public String gitAvanzadoErrorOperacion() {
		return "L'opération Git n'a pas pu être terminée. Consultez l'onglet Activité et le journal de CrashDetector.";
	}
	...
	public String gitAvanzadoActividadActualizado() {
		return "État et historique actualisés.";
	}

	@Override
	public String gitAvanzadoActividadFetch() {
		return "Récupération des références distantes...";
	}

	@Override
	public String gitAvanzadoActividadPull() {
		return "Téléchargement et intégration des modifications...";
	}

	@Override
	public String gitAvanzadoActividadPush() {
		return "Envoi des commits vers le dépôt distant...";
	}

	@Override
	public String gitAvanzadoActividadPreparar() {
		return "Indexation des fichiers sélectionnés...";
	}

	@Override
	public String gitAvanzadoActividadPrepararTodo() {
		return "Indexation de toutes les modifications...";
	}

	@Override
	public String gitAvanzadoActividadQuitarPreparacion() {
		return "Désindexation des fichiers sélectionnés...";
	}

	@Override
	public String gitAvanzadoActividadQuitarTodo() {
		return "Vidage de la zone d'index...";
	}

	@Override
	public String gitAvanzadoActividadCommit() {
		return "Création du commit...";
	}

	@Override
	public String gitAvanzadoActividadCrearRama(String rama) {
		return "Création et basculement vers la branche « " + rama + " »...";
	}

	@Override
	public String gitAvanzadoActividadCambiarRama(String rama) {
		return "Basculement vers la branche « " + rama + " »...";
	}
	...
	public String nombreVerificacionMeltdown() {
		return TraduccionesSeguridadCPU.texto("fr", "check_meltdown");
	}

	@Override
	public String nombreVerificacionSpectre() {
		return TraduccionesSeguridadCPU.texto("fr", "check_spectre");
	}

	@Override
	public String desactivarVerificacionMeltdown() {
		return TraduccionesSeguridadCPU.texto("fr", "disable_meltdown");
	}

	@Override
	public String desactivarVerificacionSpectre() {
		return TraduccionesSeguridadCPU.texto("fr", "disable_spectre");
	}

	@Override
	public String abrirDocumentacionOficialSeguridadCPU() {
		return TraduccionesSeguridadCPU.texto("fr", "open_docs");
	}

	@Override
	public String errorComprobacionSeguridadCPU(String verificacion, String detalle) {
		return TraduccionesSeguridadCPU.texto("fr", "check_error", verificacion, detalle);
	}
	...
	public String tituloProteccionProcesador(String vulnerabilidad) {
		return TraduccionesSeguridadCPU.texto("fr", "title_protection", vulnerabilidad);
	}

	@Override
	public String etiquetaSeguridadCpuCPU() {
		return TraduccionesSeguridadCPU.texto("fr", "label_cpu");
	}

	@Override
	public String etiquetaSeguridadCpuFabricante() {
		return TraduccionesSeguridadCPU.texto("fr", "label_vendor");
	}

	@Override
	public String etiquetaSeguridadCpuArquitectura() {
		return TraduccionesSeguridadCPU.texto("fr", "label_arch");
	}

	@Override
	public String etiquetaSeguridadCpuSistemaOperativo() {
		return TraduccionesSeguridadCPU.texto("fr", "label_os");
	}

	@Override
	public String etiquetaSeguridadCpuAfectado() {
		return TraduccionesSeguridadCPU.texto("fr", "label_affected");
	}

	@Override
	public String etiquetaSeguridadCpuEstadoGeneral() {
		return TraduccionesSeguridadCPU.texto("fr", "label_overall");
	}

	@Override
	public String etiquetaSeguridadCpuMitigacionSO() {
		return TraduccionesSeguridadCPU.texto("fr", "label_os_mitigation");
	}

	@Override
	public String etiquetaSeguridadCpuMicrocodigoFirmware() {
		return TraduccionesSeguridadCPU.texto("fr", "label_microcode");
	}

	@Override
	public String etiquetaSeguridadCpuRevisionDetectada() {
		return TraduccionesSeguridadCPU.texto("fr", "label_revision");
	}

	@Override
	public String etiquetaSeguridadCpuNota() {
		return TraduccionesSeguridadCPU.texto("fr", "label_note");
	}

	@Override
	public String etiquetaSeguridadCpuEvidencia() {
		return TraduccionesSeguridadCPU.texto("fr", "label_evidence");
	}

	@Override
	public String etiquetaSeguridadCpuAccionRecomendada() {
		return TraduccionesSeguridadCPU.texto("fr", "label_action");
	}

	@Override
	public String estadoAfectacionSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoAfectacion("fr", estado);
	}

	@Override
	public String estadoMitigacionSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoMitigacion("fr", estado);
	}

	@Override
	public String estadoComponenteSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoComponente("fr", estado);
	}

	@Override
	public String notaSeguridadCpuRegistroRemoto() {
		return TraduccionesSeguridadCPU.texto("fr", "remote_note");
	}

	@Override
	public String mensajeSeguridadCPU(String clave, String... argumentos) {
		return TraduccionesSeguridadCPU.mensaje("fr", clave, argumentos);
	}
	...
	public String busquedaBinariaModsBotonLateral() {
		return "Recherche binaire des mods";
	}

	@Override
	public String busquedaBinariaModsTitulo() {
		return "Recherche binaire des mods — Anya Nyabyss";
	}

	@Override
	public String busquedaBinariaModsDescripcion() {
		return "Désactive temporairement environ la moitié des mods, relance le jeu avec CDLauncher et réduit la liste des suspects selon le résultat de chaque test.";
	}

	@Override
	public String busquedaBinariaModsAdvertenciaVariosMods() {
		return "Important : cette recherche suppose qu’un seul mod provoque le problème. Elle n’est pas fiable si plusieurs mods le provoquent séparément ou si la panne dépend d’une interaction entre plusieurs mods.";
	}

	@Override
	public String busquedaBinariaModsProteccionDependencias() {
		return "Protection des dépendances : si une dépendance est désactivée, ses dépendants le sont aussi. Aucun mod laissé actif ne perdra une dépendance détectée.";
	}
	...
	public String busquedaBinariaModsEstado(String clave, int ronda, int candidatos, int desactivados, String detalle) {
		...
		switch (clave) {
		case "sin_sesion":
			return "Aucune recherche n’est active.";
		case "recuperacion":
			return "Une session interrompue existe. Restaurez les mods avant de continuer.";
		case "escaneando":
			return "Analyse des mods et des dépendances…";
		case "juego_activo":
			return "Un jeu relancé est déjà en cours.";
		case "restaurando":
			return "Restauration des mods…";
		case "preparando":
			return "Préparation de la manche " + ronda + " (" + candidatos + " / " + desactivados + ")";
		case "lanzando":
			return "Relance du jeu pour la manche " + ronda + " (" + candidatos + " / " + desactivados + ")";
		case "juego":
			return "Jeu en cours pour la manche " + ronda + " (" + candidatos + " / " + desactivados + ")";
		case "esperando":
			return "Le jeu est terminé. Indiquez si le problème persiste ou a disparu.";
		case "ronda_persiste":
			return "Manche marquée : le problème persiste.";
		case "ronda_desaparece":
			return "Manche marquée : le problème a disparu.";
		case "restaurado":
			return "Tous les mods ont été restaurés.";
		case "sin_mods":
			return "Au moins deux mods actifs sont nécessaires.";
		case "sin_division":
			return "L’ensemble restant ne peut pas être divisé sans casser les dépendances détectées.";
		case "inconsistente":
			return "Les résultats sont incohérents. Cela indique souvent plusieurs causes ou une interaction entre mods.";
		default:
			return detalle == null ? "" : detalle;
		}
	}
	...
	public String busquedaBinariaModsConfirmacion() {
		return "L’outil renommera temporairement les fichiers de mods et relancera le jeu. Un manifeste de récupération sera créé. Continuer ?";
	}

	@Override
	public String busquedaBinariaModsResultadoUnico(String mod) {
		return "Mod suspect isolé : " + mod;
	}

	@Override
	public String busquedaBinariaModsResultadoGrupo(String mods) {
		return "Ce groupe de dépendances n’a pas pu être séparé : " + mods;
	}

	@Override
	public String busquedaBinariaModsError(String detalle) {
		return "Erreur de recherche binaire : " + detalle;
	}
	...
	public String busquedaBinariaModsColor(String clave) {
		...
		case "exito":
			return "Couleur de réussite";
		case "borde":
			return "Couleur des bordures";
		case "seleccion":
	...
	public String mensajeWatchdogCierreClientePostMain() {
		return "<b style='color:#" + config.obtenerColorAdvertencia() + "'>"
				+ "Minecraft a mis trop de temps à se fermer.</b>" + "<p>Le jeu a généré :</p>"
				+ "<code>java.lang.Error: Watchdog (Client shutdown from post-main)</code>"
				+ "<p>Ce rapport apparaît lorsque le processus de Minecraft reste actif pendant "
				+ "environ 15 secondes après le début de la fermeture.</p>"
				+ "<p><b>Cela ne représente normalement pas un problème grave en soi.</b> "
				+ "Cela peut signifier qu'un mod, un thread ou une tâche de nettoyage a mis trop de temps "
				+ "à se terminer pendant la fermeture.</p>"
				+ "<p>Il peut également s'agir d'un rapport secondaire généré alors que le jeu "
				+ "était déjà en train de se fermer après un autre plantage.</p>" + "<p><b>Que faire :</b></p>" + "<ul>"
				+ "<li>Si le jeu s'est fermé correctement et que cela ne s'est produit qu'une fois, vous pouvez l'ignorer.</li>"
				+ "<li>Si cela se reproduit, vérifiez les erreurs précédentes dans " + "<code>latest.log</code>.</li>"
				+ "<li>Recherchez les mods qui maintiennent des tâches, des connexions ou des processus actifs pendant la fermeture.</li>"
				+ "<li>Ne supposez pas que le dernier thread affiché dans le dump est nécessairement le responsable.</li>"
				+ "</ul>";
	}

	@Override
	public String nombreWatchdogCierreClientePostMain() {
		return "Fermeture lente du client (Watchdog post-main)";
	}
