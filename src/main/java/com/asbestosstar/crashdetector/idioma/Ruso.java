...
public class Ruso implements Idioma {
	...
	public String javaProblematica() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Обнаружен нативный краш среды выполнения Java (JVM).</b>"
				+ "<p>Этот тип ошибки возникает внутри самой виртуальной машины Java (например, в <code>jvm.dll</code>, <code>libjvm.so</code> и т.д.), "
				+ "и не обязательно вызван модом.</p>"
				+ "<p>Хотя в редких случаях это может быть вызвано модами, использующими несовместимые нативные библиотеки, "
				+ "<b>гораздо более вероятно, что это связано с дефектной, поврежденной или устаревшей версией JVM</b>.</p>"
				+ "<p>Это особенно часто встречается, если вы используете старую или неофициальную сборку вашей версии Java (например,.community-сборки без поддержки).</p>"
				+ "<p><b>Рекомендуем использовать надежную и поддерживаемую JVM:</b></p>" + "<ul>"
				+ "<li><b>Red Hat Build of OpenJDK</b> (стабильная, хорошо протестированная, идеальна для Windows/Linux)</li>"
				+ "<li><b>OpenLogic OpenJDK</b> (кроссплатформенная поддержка, включая macOS Intel)</li>"
				...
				+ "<p>Избегайте неизвестных, кастомных или очень старых сборок, так как они могут содержать критические ошибки в движке JVM.</p>"
				+ "<p><b>Используете Prism Launcher или TLauncher?</b> Настроить пользовательскую JVM очень просто: "
				+ "в Prism Launcher перейдите в <i>Installations</i> → <i>Edit Instance</i> → <i>Java Settings</i>; "
				+ "в TLauncher перейдите в <i>Settings</i> → <i>Java Settings</i> и выберите путь к скачанному JDK/JRE. "
				+ "Также возможно, что ваш сборщик мусора испытывает проблемы; в таком случае вам следует переключиться на ZGC."
	...
	public String mensajeOculusIrisUnknownShaderVariable() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "<p><b>Рекомендуемый порядок действий:</b></p>" + "<ul>"
	...
	public String mensajeItemNoExiste(String itemFaltante, String namespace) {
		...
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Попытка использовать несуществующий предмет.</b>" + "<p>Журнал содержит строку <b>Item: " + itemHtml
				+ " does not exist</b>.</p>"
				+ "<p>Это обычно означает, что какой-то <b>датапак</b>, <b>мод</b> или <b>конфигурация</b> "
				+ "ссылается на предмет, которого нет в игре.</p>" + "<p><b>Что проверить:</b></p>" + "<ul>"
				...
				+ "<li>Если этого мода нет, попробуйте <b>установить его</b>.</li>" + "</ul>"
				+ "<p><b>Чтобы узнать, какой мод или датапак запрашивает этот предмет:</b></p>" + "<ul>"
				+ "<li>Используйте утилиту <b>grepr</b> на боковой панели.</li>" + "<li><b>Не</b> выбирайте папку.</li>"
				+ "<li>Включите опцию <b>search in archives</b>.</li>"
				+ "<li>В поле поиска введите <b>namespace</b>, то есть часть перед двоеточием: " + "<b>" + namespaceHtml
				+ "</b>.</li>" + "</ul>"
	...
	public String mensajeColdSweatInitDynamicTags() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "или конфликтной комбинации с другим модом или датапаком.</p>" + "<p><b>Что попробовать:</b></p>"
				+ "<ul>" + "<li>Удалите или отключите <b>Cold Sweat</b> и попробуйте снова.</li>"
				...
				+ "<li>Если ошибка началась после обновления модов, попробуйте другую комбинацию версий.</li>" + "</ul>"
	...
	public String mensajeClassCastExceptionGeneral(String lineaClassCast) {
		String detalle = lineaClassCast == null || lineaClassCast.isEmpty() ? ""
				: "<p><b>Обнаруженная строка:</b></p><p><code>" + lineaClassCast + "</code></p>";

		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Обнаружен ClassCastException.</b>"
				+ "<p>Это означает, что класс был обработан как другой, несовместимый класс или интерфейс.</p>"
				+ detalle + "<p>Такая ошибка обычно вызвана одной из следующих причин:</p>" + "<ul>"
				+ "<li><b>Два несовместимых мода</b> между собой.</li>"
				+ "<li><b>Mixins</b>, <b>transformers</b> или патчи, изменяющие класс, из-за чего другая часть игры ожидает другой тип данных.</li>"
				+ "<li>Другие моды, указанные в <b>stacktrace</b>, которые вызывают некорректное приведение типов.</li>"
				+ "</ul>" + "<p><b>Что проверить:</b></p>" + "<ul>"
				+ "<li>Просмотрите строки <b>stacktrace</b>, связанные с этой ошибкой.</li>"
				+ "<li>Обратите особое внимание на имена модов или классов в формате <b>$modid$algo</b>, так как они обычно указывают на задействованные моды.</li>"
				+ "<li>Попробуйте обновить, удалить или отделить моды, которые, вероятно, связаны с неверным приведением типов.</li>"
				+ "</ul>" + "<p>Хотя <b>ClassCastException</b> не всегда приводит к вылету, очень часто это так.</p>";
	}

	@Override
	public String nombreClassCastExceptionGeneral() {
		return "Обнаружен ClassCastException";
	}
	...
	public String mensajeVSTournamentVSConfigClassNoExiste() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "VS Tournament, похоже, слишком стар для вашей версии Valkyrien Skies.</b>"
				+ "<p>Журнал содержит <b>NoClassDefFoundError</b> для "
				+ "<b>org/valkyrienskies/core/impl/config/VSConfigClass</b>, а также строку от "
				+ "<b>org.valkyrienskies.tournament.TournamentMod.init(...)</b>.</p>"
				+ "<p>Обычно это означает, что у вас <b>старая версия VS Tournament</b>, которая пытается "
				+ "использовать старые внутренние классы <b>Valkyrien Skies</b>, которых больше не существует.</p>"
				+ "<p><b>Что сделать:</b></p>" + "<ul>" + "<li>Удалите старый <b>VS Tournament</b>.</li>"
				+ "<li>Вместо него используйте <b>VS Tournament Reforged</b>.</li>"
				+ "<li>Кроме того, проверьте, что версия <b>Valkyrien Skies</b> совпадает с версией, поддерживаемой аддоном.</li>"
				+ "</ul>"
				+ "<p>Рекомендация перейти на <b>VS Tournament Reforged</b> соответствует текущему состоянию проекта: "
				+ "оригинальная версия Tournament всё ещё числится как старый мод для 1.18.2, тогда как "
				+ "<b>VS Tournament Reforged</b> публикуется отдельно и в настоящее время заявляет поддержку Valkyrien "
	...
	public String mensajeControlifyRemoveReloadingScreen() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Обнаружена несовместимость между Controlify и Remove Reloading Screen.</b>"
				+ "<p>Журнал содержит строки <b>Attempted to fetch default config before DefaultConfigManager was ready!</b> "
				+ "и <b>$rrls$init</b>, что обычно указывает на конфликт между <b>Controlify</b> и "
	...
	public String mensajeBiomesOPlentyFogDataLiquidosPersonalizados() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Возможная проблема с Biomes O' Plenty и пользовательскими жидкостями.</b>"
				+ "<p>Журнал содержит ошибку <b>class org.joml.Vector4f cannot be cast to class "
	...
	public String mensajeKotlinReflectionInternalErrorVersion() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Обнаружена внутренняя ошибка рефлексии Kotlin.</b>"
				+ "<p>Журнал содержит <b>kotlin.reflect.jvm.internal.KotlinReflectionInternalError</b> с сообщением, похожим на "
	...
	public String mensajeEntradaDuplicadaIdModerno() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Обнаружен дубликат записи в реестре Minecraft.</b>"
				+ "<p>Лог содержит ошибку, похожую на <b>Duplicate entry on id</b>, например "
				+ "<b>current=maroon, previous=mint</b>.</p>"
				+ "<p>В современных версиях Minecraft такая ошибка обычно возникает, когда два мода пытаются зарегистрировать "
				+ "разные записи, используя один и тот же внутренний ID.</p>" + "<p><b>Рекомендуемые действия:</b></p>"
				+ "<ul>" + "<li>Удалите один из модов, регистрирующих дублирующуюся запись.</li>"
				+ "<li>Если вы узнаете имена, упомянутые в ошибке, проверьте, какой мод добавляет эти записи, и попробуйте запустить игру без него.</li>"
				+ "<li>Если вы не узнаете имена, используйте утилиту <b>grepr</b> на боковой панели.</li>"
				+ "<li>В <b>grepr</b> включите поиск внутри сжатых файлов <b>.jar</b>, <b>.zip</b> и <b>.fpm</b>.</li>"
				+ "<li>Также включите поиск в <b>бинарных файлах</b>, так как некоторые имена или ID могут находиться внутри скомпилированных классов.</li>"
				+ "</ul>"
				+ "<p>Ищите значения, упомянутые в ошибке, такие как <b>maroon</b> или <b>mint</b>, чтобы найти, какой мод их содержит.</p>";
	}

	@Override
	public String nombreEntradaDuplicadaIdModerno() {
		return "Дубликат записи в ID мода";
	}

	@Override
	public String nombreOpenGLMemoriaInsuficiente() {
		return "OpenGL – недостаточно видеопамяти";
	}

	@Override
	public String mensajeOpenGLMemoriaInsuficiente() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Minecraft выдал ошибку OpenGL из-за нехватки графической памяти.</b>" + "<p>Игра сообщила:</p>"
				+ "<code>GL_OUT_OF_MEMORY</code>"
				+ "<p>Это обычно означает, что видеокарта или система не смогли выделить достаточно памяти для текстур, шейдеров, моделей, буферов или визуальных эффектов.</p>"
				+ "<p><b>Распространенные причины:</b></p>" + "<ul>" + "<li>Слишком тяжелые шейдеры.</li>"
				+ "<li>Наборы ресурсов (Resource packs) высокого разрешения.</li>"
				+ "<li>Слишком много визуальных модов или модов на рендеринг.</li>"
				+ "<li>Слишком большая дальность прорисовки.</li>" + "<li>Недостаточно доступной VRAM.</li>"
				+ "<li>Устаревшие или нестабильные драйверы видеокарты.</li>" + "</ul>"
				+ "<p><b>Рекомендуемое решение:</b></p>" + "<ul>" + "<li>Временно отключить шейдеры.</li>"
				+ "<li>Использовать наборы ресурсов с меньшим разрешением.</li>"
				+ "<li>Уменьшить дальность прорисовки и симуляции.</li>"
				+ "<li>Снизить качество графики, тени, частицы и мипмапы.</li>"
				+ "<li>Обновить драйверы видеокарты.</li>"
				+ "<li>Закрыть другие программы, использующие GPU или много памяти.</li>" + "</ul>"
	...
	public String error_al_cargar_plantilla_desde_disco() {
		return "Ошибка загрузки шаблона с диска: ";
	}

	public String no_se_encontro_plantilla_restablecer() {
		return "Шаблон не найден. Восстановите, используя кнопку 'Restablecer Plantilla'.";
	}

	public String plantilla_guardada_en() {
		return "Шаблон сохранен в: ";
	}

	public String plantilla_restablecida_correctamente() {
		return "Шаблон успешно восстановлен.";
	}
	...
	public String error_al_restablecer_imagen() {
		return "Ошибка восстановления изображения: ";
	}

	public String no_se_encontro_imagen_en_recurso() {
		return "Изображение не найдено в ресурсах: ";
	}

	public String imagen_restablecida() {
		return "Изображение восстановлено: ";
	}
	...
	public String ver_codigo() {
		return "Просмотр кода";
	}

	public String importar_instancia() {
		return "Импортировать инстанс";
	}

	public String compartir_instancia() {
		return "Поделиться инстансом";
	}

	public String error_al_cargar_mods() {
		return "Ошибка загрузки модов.";
	}
	...
	public String guardar_como_archivo() {
		return "Сохранить как файл";
	}

	public String exportando_modpack() {
		return "Экспорт модпака...";
	}

	public String modpack_exportado() {
		return "Модпак экспортирован:\n";
	}
	...
	public String descargar_deps() {
		return "Скачать зависимости";
	}

	public String no_faltan_dependencias() {
		return "Зависимости не отсутствуют.";
	}

	public String descargar_nbt_para_quests() {
		return "Скачать NBT ДЛЯ КВЕСТОВ";
	}
	...
	public String error_exportando_modpack() {
		return "Ошибка экспорта модпака:\n";
	}

	@Override
	public String importador_confirmar_descargar_nbt_para_quests() {
		return "Будут загружены необходимые зависимости NBT для слияния квестов.\n\n"
				+ "После этого может потребоваться перезапуск, чтобы они попали в classpath.";
	}
	...
	public String dependencia_nbt_descargada_reiniciar(String nombrePrograma) {
		return "Зависимость NBT загружена.\n\n" + "Перезапустите " + nombrePrograma
				+ ", если слияние SNBT все еще сообщает об отсутствии движка NBT.";
	}
	...
	public String migradorCAAvisoNoMigrado() {
		return "Предупреждение: из-за различий в типах конфигурации черный список проверок не может быть перенесен автоматически. Оттуда вы также можете отключить предупреждения Intel и Mod List Diff; так же, как и в CrashDetector, они считаются обычными проверками. Если вы не включите примитивный режим, вам нужно будет использовать редактор HTML WYSIWYG в настройках CrashDetector для вставки ссылок. Если вы используете примитивный режим, ссылка помощи будет скопирована автоматически из Crash Assistant. Чтобы изменить цвета и настройки в графическом интерфейсе (GUI), вам нужно перейти в настройки CrashDetector, зайти в область CDSkinCape/GUI Editor и выбрать тип GUI, а также его конкретную реализацию. Изображения можно редактировать из следующего местоположения: "
				+ Statics.carpeta.resolve("imagenes").toString()
				+ ". Скрипты JEXL также не будут перенесены. Если вам нужно только выполнять базовый анализ с помощью сопоставления строк или регулярных выражений, вы можете использовать функцию 'Agregar Razones' в корпоративной конфигурации; если вам нужны более продвинутые функции, вам следует создать расширение на Java: https://github.com/FeatureCreepEAP/crashdetector-tutorial-extention-english";
	}
	...
	public String migradorCAModoPrincipalAviso() {
		return "Выберите, как мигрировать главный интерфейс. 'centro_soporte' наиболее похож на интерфейс по умолчанию Crash Assistant, но не рекомендуется, особенно для пользователей DIY, так как имеет множество всплывающих окон и плохой дизайн; он существует только для помощи в миграции устаревших сред Crash Assistant. 'primitiva' — это очень простой интерфейс, используемый только для обмена; рекомендуется только для модпаков, а не для пользователей DIY, если вы отключите все ненужные проверки, и он также страдает от множества всплывающих окон. 'original' сохраняет ваш текущий главный интерфейс и обычно является рекомендуемым вариантом для большинства пользователей.";
	}

	@Override
	public String migradorCASitioLoggingAviso() {
		return "Выберите, хотите ли вы мигрировать сайт логирования из Crash Assistant или сохранить текущий из CrashDetector. Crash Assistant часто использует gnomebot.dev, который не рекомендуется по сравнению с SecureLogger, PastesDev или CDPaste. Для большинства пользователей рекомендуется сохранить текущий сайт.";
	}

	@Override
	public String ideScriptTitulo() {
		return "IDE для скриптов";
	}
	...
	public String ideScriptProyectoDeshabilitadoAviso() {
		return "Этот тип проекта уже существует в интерфейсе, но отключен до добавления его языкового сервера или анализатора.";
	}
	...
	public String ideScriptConfirmarDescargaDeps(int cantidad, String lista) {
		return "Будет загружено " + cantidad + " зависимостей для IDE скриптов:\n\n" + lista
				+ "\nПосле этого может потребоваться перезапуск, чтобы они попали в classpath.";
	}
	...
	public String ideScriptDepsDescargadas(String mensaje) {
		return "Результат загрузки:\n\n" + mensaje
				+ "\n\nПерезапустите программу, если классы все еще не отображаются в classpath.";
	}
	...
	public String mcpTituloPrincipal() {
		return "Сервер MCP для локального ИИ";
	}
	...
	public String mcpDescargarDependencias() {
		return "Скачать зависимости MCP/CFR";
	}

	public String mcpIniciarServidor() {
		return "Запустить сервер MCP";
	}

	public String mcpEstadoDependenciasNoCargadas() {
		return "Зависимости MCP/CFR не загружены. Скачайте их и перезапустите, если кнопка все еще неактивна.";
	}

	public String mcpEstadoDependenciasCargadas() {
		return "Зависимости MCP/CFR обнаружены. Сервер MCP может быть запущен.";
	}

	public String mcpDependenciasDescargadasReiniciar() {
		return "Зависимости скачаны. Если они все еще не загружены, перезапустите CrashDetector.";
	}

	public String mcpErrorDescargandoDependencias(String error) {
		return "Не удалось скачать зависимости MCP/CFR: " + error;
	}

	public String mcpServidorIniciado(int puerto) {
		return "Сервер MCP запущен на 127.0.0.1:" + puerto + "/mcp";
	}

	public String mcpErrorIniciandoServidor(String error) {
		return "Не удалось запустить сервер MCP: " + error;
	}

	public String mcpImagenNoDisponible() {
		return "Изображение MCP недоступно";
	}
	...
	public String mcpDescripcionHtml() {
		return "<b>Эта панель запускает локальный сервер MCP для ассистентов, таких как Claude Desktop, Qwen Desktop Linux, Red Hat command-line-assistant или Goose.</b>"
				+ "<br><br>"
				+ "Сначала скачайте зависимости MCP/CFR. Затем перезапустите, если классы не появятся в classpath."
				+ "<br><br>" + "<b>Claude Desktop / Qwen Desktop Linux, пример:</b>" + "<pre>{\n"
				+ "  \"mcpServers\": {\n" + "    \"crashdetector\": {\n"
				+ "      \"url\": \"http://127.0.0.1:8765/mcp\"\n" + "    }\n" + "  }\n" + "}</pre>"
				+ "<b>Goose / command-line-assistant:</b>" + "<pre>http://127.0.0.1:8765/mcp</pre>"
				+ "Пока этот сервер MCP базовый. Позже можно добавить реальные инструменты для чтения логов, запросов к CFR, проверки модов, поиска классов и объяснения крашей.";
	}
	...
	public String mensajeErrorJvmDllCurseForgeG1(boolean conOverwolf) {
		...
		if (conOverwolf) {
			textoOverwolf = "<p>Лог также показывает признаки <b>Overwolf</b> или связанных DLL, таких как "
					+ "<b>OWClient.dll</b> или <b>OWUtils.dll</b>. Это само по себе не доказывает, что Overwolf является причиной, "
					+ "но указывает на то, что процесс Minecraft выполнялся в более модифицированной среде, "
					+ "чем обычный запуск vanilla.</p>";
		}

		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Java закрылась из-за нативного сбоя в jvm.dll во время работы сборщика G1 в CurseForge.</b>"
				+ "<p>Лог содержит <b>EXCEPTION_ACCESS_VIOLATION</b> внутри <b>jvm.dll</b>, "
				+ "и активный поток — это <b>GCTaskThread</b>. Это означает, что Java упала, пока сборщик мусора "
				+ "<b>G1</b> работал.</p>"
				+ "<p>Также есть признаки того, что игра была запущена из среды, управляемой "
				+ "<b>CurseForge</b>, например, пути CurseForge, <b>DCFInstanceId</b> или метка лаунчера CurseForge.</p>"
				+ textoOverwolf
				+ "<p>Невозможно доказать только по логу, что CurseForge или Overwolf являются точной причиной. "
				+ "Однако этот паттерн часто встречается в запусках CurseForge и может быть связан "
				+ "с комбинацией <b>G1 GC</b>, нативной памяти, библиотек Minecraft, графических драйверов, "
				+ "нативных папок, управляемых CurseForge, оверлеев или внешних компонентов лаунчера.</p>"
				+ "<p><b>Общее решение:</b></p>" + "<ul>"
				+ "<li>Смените сборщик мусора, добавив этот аргумент JVM: <b>-XX:+UseShenandoahGC</b></li>"
				+ "<li>В CurseForge это обычно добавляется в дополнительные аргументы Java профиля или лаунчера.</li>"
				+ "<li>Очистите кэш natives/bin профиля CurseForge, чтобы принудительно извлечь нативные библиотеки заново.</li>"
				+ "<li>Обновите Java и графические драйверы.</li>"
				+ "<li>Если вы используете оверлеи, программы записи, агрессивные антивирусы или другие инструменты, внедряющиеся в игры, попробуйте временно их закрыть.</li>"
				+ "</ul>" + "<p>Рекомендуемое руководство на английском для изменения аргументов в CurseForge: "
				+ "<a href='https://youtu.be/UKFWBOZxB2o'>https://youtu.be/UKFWBOZxB2o</a></p>"
				+ "<p><b>Примечание для Minecraft 1.16.5 или старше:</b> эти версии обычно используют Java 8. "
				+ "Если вы используете JDK 8 и хотите использовать Shenandoah, вам, возможно, потребуется использовать "
				+ "<b>Red Hat Build of OpenJDK 8</b> и указать CurseForge на эту установку Java.</p>"
				+ "<p>Руководство Red Hat по установке OpenJDK 8 в Windows: "
	...
	public String nombreErrorJvmDllCurseForgeG1() {
		return "Сбой jvm.dll CurseForge с G1 GC";
	}

	@Override
	public String mensajeErrorJvmDllC2Sodium() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Java закрылась из-за нативного сбоя в jvm.dll во время компиляции кода Sodium или подобного мода.</b>"
				+ "<p>Лог содержит <b>EXCEPTION_ACCESS_VIOLATION</b> внутри <b>jvm.dll</b>, "
				+ "и активный поток — это <b>C2 CompilerThread</b>. Это означает, что сбой произошел внутри "
				+ "JIT-компилятора Java, а не как обычное исключение Minecraft.</p>"
				+ "<p>В этом случае задача компиляции упоминает код, связанный с <b>Sodium</b>, "
				+ "<b>Embeddium</b>, <b>Rubidium</b> или подобным классом, таким как "
				+ "<b>ClonedChunkSectionCache::acquire</b>.</p>" + "<p><b>Рекомендуемое решение:</b></p>" + "<ul>"
				+ "<li>Обновите <b>Sodium</b>, <b>Embeddium</b>, <b>Rubidium</b>, <b>Oculus</b> или любой связанный мод рендеринга.</li>"
				+ "<li>Обновите Java 17. Если вы уже используете современную версию, попробуйте другой дистрибутив, такой как Temurin, Zulu или Microsoft OpenJDK.</li>"
				+ "<li>Обновите драйверы видеокарты.</li>"
				+ "<li>Попробуйте временно без Sodium, Embeddium, Rubidium или Oculus, чтобы подтвердить, исчезнет ли сбой.</li>"
				+ "<li>В качестве продвинутого теста используйте <b>-XX:TieredStopAtLevel=1</b>, чтобы снизить использование компилятора C2. "
				+ "Это может снизить производительность, но помогает подтвердить, что сбой исходит от JIT-компилятора.</li>"
				+ "</ul>" + "<p>Эта проблема не то же самое, что обычный краш мода. Она также отличается от сбоев, где "
				+ "<b>GCTaskThread</b> появляется как активный поток. Здесь паттерн больше указывает на компилятор C2 Java, "
				+ "компилирующий оптимизированный код рендеринга.</p>";
	}

	@Override
	public String nombreErrorJvmDllC2Sodium() {
		return "Нативный сбой Java с Sodium / Embeddium";
	}
	...
	public String mensajeErrorArchivoUsadoPorOtroProceso(String archivo) {
		...
		if (archivo != null && archivo.length() > 0) {
			textoArchivo = "<p><b>Файл заблокирован:</b> " + archivo + "</p>";
		}

		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Файл используется другим процессом.</b>"
				+ "<p>Лог содержит ошибку <b>java.nio.file.FileSystemException</b>, указывающую, что Windows не может "
				+ "получить доступ к файлу, так как он используется другой программой.</p>" + textoArchivo
				+ "<p>Это обычно происходит, когда Minecraft, CurseForge, текстовый редактор, антивирус, OneDrive, "
				+ "программа синхронизации или даже другой экземпляр игры держат файл открытым.</p>"
				+ "<p><b>Рекомендуемое решение:</b></p>" + "<ul>" + "<li>Полностью закройте Minecraft.</li>"
				+ "<li>Закройте CurseForge и откройте его снова.</li>"
				+ "<li>Проверьте Диспетчер задач и завершите дублирующиеся процессы <b>javaw.exe</b>, <b>java.exe</b>, Minecraft или CurseForge.</li>"
				+ "<li>Закройте текстовые редакторы, которые могут держать открытый файл конфигурации.</li>"
				+ "<li>Если файл находится в OneDrive, Dropbox или подобной программе, временно приостановите синхронизацию.</li>"
				+ "<li>Если проблема сохраняется, перезагрузите компьютер, чтобы снять блокировку файла.</li>" + "</ul>"
				+ "<p>После освобождения файла снова запустите модпак. Если заблокированный файл является конфигурацией, "
				+ "также может помочь создание резервной копии и регенерация этого файла.</p>";
	}
	...
	public String gitAvanzadoTipo() {
		return "Расширенный Git-клиент";
	}

	@Override
	public String jgitAbrirClienteAvanzado() {
		return "Открыть расширенный Git-клиент";
	}

	@Override
	public String gitAvanzadoTitulo() {
		return "Расширенный Git-клиент — Wendy Mizumi: Lost Media";
	}

	@Override
	public String gitAvanzadoSerieLostMedia() {
		return "Wendy Mizumi — Lost Media / архив текущего репозитория";
	}
	...
	public String gitAvanzadoHistorial() {
		return "История коммитов";
	}

	@Override
	public String gitAvanzadoNoPreparados() {
		return "Неподготовленные изменения";
	}
	...
	public String gitAvanzadoActualizando() {
		return "Обновление текущего репозитория...";
	}

	@Override
	public String gitAvanzadoJGitNoDisponible() {
		return "JGit недоступен. Установите зависимости через обычный интерфейс Git и перезапустите приложение.";
	}

	@Override
	public String gitAvanzadoRepositorioRequerido() {
		return "Расширенный клиент можно открыть только после создания репозитория текущего экземпляра через обычный интерфейс Git.";
	}

	@Override
	public String gitAvanzadoEstadoResumen(int noPreparados, int preparados, int commits) {
		return "Неподготовлено: " + noPreparados + " | Подготовлено: " + preparados + " | Показано коммитов: "
				+ commits;
	}
	...
	public String gitAvanzadoNombreNuevaRama() {
		return "Имя новой ветки:";
	}

	@Override
	public String gitAvanzadoConfirmarCambioRama(String rama) {
		return "Переключиться на ветку «" + rama
				+ "»? Локальные изменения должны быть совместимы с переключением ветки.";
	}

	@Override
	public String gitAvanzadoErrorOperacion() {
		return "Не удалось завершить операцию Git. Проверьте вкладку «Активность» и журнал CrashDetector.";
	}
	...
	public String gitAvanzadoActividadActualizado() {
		return "Состояние и история обновлены.";
	}

	@Override
	public String gitAvanzadoActividadFetch() {
		return "Получение ссылок удалённого репозитория...";
	}

	@Override
	public String gitAvanzadoActividadPull() {
		return "Загрузка и интеграция изменений...";
	}

	@Override
	public String gitAvanzadoActividadPush() {
		return "Отправка коммитов в удалённый репозиторий...";
	}

	@Override
	public String gitAvanzadoActividadPreparar() {
		return "Подготовка выбранных файлов...";
	}

	@Override
	public String gitAvanzadoActividadPrepararTodo() {
		return "Подготовка всех изменений...";
	}

	@Override
	public String gitAvanzadoActividadQuitarPreparacion() {
		return "Удаление выбранных файлов из индекса...";
	}

	@Override
	public String gitAvanzadoActividadQuitarTodo() {
		return "Очистка индекса...";
	}
	...
	public String gitAvanzadoActividadCrearRama(String rama) {
		return "Создание и переключение на ветку «" + rama + "»...";
	}

	@Override
	public String gitAvanzadoActividadCambiarRama(String rama) {
		return "Переключение на ветку «" + rama + "»...";
	}
	...
	public String gitAvanzadoColorFondo() {
		return "Расширенный Git — фон Lost Media";
	}

	@Override
	public String gitAvanzadoColorPanel() {
		return "Расширенный Git — панель";
	}

	@Override
	public String gitAvanzadoColorPanelSecundario() {
		return "Расширенный Git — дополнительная панель";
	}

	@Override
	public String gitAvanzadoColorTexto() {
		return "Расширенный Git — текст";
	}

	@Override
	public String gitAvanzadoColorTextoSuave() {
		return "Расширенный Git — приглушённый текст";
	}

	@Override
	public String gitAvanzadoColorAzul() {
		return "Расширенный Git — архивный синий";
	}

	@Override
	public String gitAvanzadoColorRosa() {
		return "Расширенный Git — ленточный розовый";
	}

	@Override
	public String gitAvanzadoColorBorde() {
		return "Расширенный Git — граница";
	}

	@Override
	public String gitAvanzadoColorTabla() {
		return "Расширенный Git — таблица";
	}

	@Override
	public String gitAvanzadoColorSeleccion() {
		return "Расширенный Git — выделение";
	}

	@Override
	public String gitAvanzadoColorDiff() {
		return "Расширенный Git — фон diff";
	}
	...
	public String nombreVerificacionMeltdown() {
		return TraduccionesSeguridadCPU.texto("ru", "check_meltdown");
	}

	@Override
	public String nombreVerificacionSpectre() {
		return TraduccionesSeguridadCPU.texto("ru", "check_spectre");
	}

	@Override
	public String desactivarVerificacionMeltdown() {
		return TraduccionesSeguridadCPU.texto("ru", "disable_meltdown");
	}

	@Override
	public String desactivarVerificacionSpectre() {
		return TraduccionesSeguridadCPU.texto("ru", "disable_spectre");
	}

	@Override
	public String abrirDocumentacionOficialSeguridadCPU() {
		return TraduccionesSeguridadCPU.texto("ru", "open_docs");
	}

	@Override
	public String errorComprobacionSeguridadCPU(String verificacion, String detalle) {
		return TraduccionesSeguridadCPU.texto("ru", "check_error", verificacion, detalle);
	}
	...
	public String tituloProteccionProcesador(String vulnerabilidad) {
		return TraduccionesSeguridadCPU.texto("ru", "title_protection", vulnerabilidad);
	}

	@Override
	public String etiquetaSeguridadCpuCPU() {
		return TraduccionesSeguridadCPU.texto("ru", "label_cpu");
	}

	@Override
	public String etiquetaSeguridadCpuFabricante() {
		return TraduccionesSeguridadCPU.texto("ru", "label_vendor");
	}

	@Override
	public String etiquetaSeguridadCpuArquitectura() {
		return TraduccionesSeguridadCPU.texto("ru", "label_arch");
	}

	@Override
	public String etiquetaSeguridadCpuSistemaOperativo() {
		return TraduccionesSeguridadCPU.texto("ru", "label_os");
	}

	@Override
	public String etiquetaSeguridadCpuAfectado() {
		return TraduccionesSeguridadCPU.texto("ru", "label_affected");
	}

	@Override
	public String etiquetaSeguridadCpuEstadoGeneral() {
		return TraduccionesSeguridadCPU.texto("ru", "label_overall");
	}

	@Override
	public String etiquetaSeguridadCpuMitigacionSO() {
		return TraduccionesSeguridadCPU.texto("ru", "label_os_mitigation");
	}

	@Override
	public String etiquetaSeguridadCpuMicrocodigoFirmware() {
		return TraduccionesSeguridadCPU.texto("ru", "label_microcode");
	}

	@Override
	public String etiquetaSeguridadCpuRevisionDetectada() {
		return TraduccionesSeguridadCPU.texto("ru", "label_revision");
	}

	@Override
	public String etiquetaSeguridadCpuNota() {
		return TraduccionesSeguridadCPU.texto("ru", "label_note");
	}

	@Override
	public String etiquetaSeguridadCpuEvidencia() {
		return TraduccionesSeguridadCPU.texto("ru", "label_evidence");
	}

	@Override
	public String etiquetaSeguridadCpuAccionRecomendada() {
		return TraduccionesSeguridadCPU.texto("ru", "label_action");
	}

	@Override
	public String estadoAfectacionSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoAfectacion("ru", estado);
	}

	@Override
	public String estadoMitigacionSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoMitigacion("ru", estado);
	}

	@Override
	public String estadoComponenteSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoComponente("ru", estado);
	}

	@Override
	public String notaSeguridadCpuRegistroRemoto() {
		return TraduccionesSeguridadCPU.texto("ru", "remote_note");
	}

	@Override
	public String mensajeSeguridadCPU(String clave, String... argumentos) {
		return TraduccionesSeguridadCPU.mensaje("ru", clave, argumentos);
	}
	...
	public String mensajeWatchdogCierreClientePostMain() {
		return "<b style='color:#" + config.obtenerColorAdvertencia() + "'>" + "Minecraft слишком долго закрывался.</b>"
				+ "<p>Игра сгенерировала:</p>"
				+ "<code>java.lang.Error: Watchdog (Client shutdown from post-main)</code>"
				+ "<p>Этот отчет появляется, когда процесс Minecraft остается активным примерно "
				+ "15 секунд после начала закрытия.</p>"
				+ "<p><b>Само по себе это обычно не представляет серьезной проблемы.</b> "
				+ "Это может означать, что мод, поток или задача очистки заняли слишком много времени "
				+ "для завершения во время закрытия.</p>"
				+ "<p>Это также может быть вторичный отчет, сгенерированный, когда игра уже "
				+ "закрывалась после другой ошибки.</p>" + "<p><b>Что делать:</b></p>" + "<ul>"
				+ "<li>Если игра закрылась корректно и это произошло только один раз, можете игнорировать.</li>"
				+ "<li>Если это происходит повторно, проверьте предыдущие ошибки в " + "<code>latest.log</code>.</li>"
				+ "<li>Ищите моды, которые поддерживают активные задачи, соединения или процессы во время закрытия.</li>"
				+ "<li>Не предполагайте, что последний поток, показанный в дампе, обязательно является виновником.</li>"
				+ "</ul>";
	}

	public String nombreWatchdogCierreClientePostMain() {
		return "Медленное закрытие клиента (Watchdog post-main)";
	}
