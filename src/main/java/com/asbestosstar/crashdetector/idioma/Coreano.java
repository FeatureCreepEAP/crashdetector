...
public class Coreano implements Idioma {
	...
	@Override
	public String arco() {
		return "이 대화상자는 SecureLogger API를 사용하여 "
				+ "<a href=\"https://securelogger.net\">securelogger.net</a>에 로그를 공유할 수 있게 해줍니다. 보고서 공유 버튼을 누르면, "
				+ "보고서가 선택된 엔드포인트(asbestosstar.egoism.jp, 기본값)로 전송됩니다(하단에서 변경 가능). "
				+ "선택한 모든 로그를 보고서와 함께 공유할 수 있습니다. 업로드를 원하지 않으시면 이 대화상자를 사용하지 마십시오! "
				+ "공식 엔드포인트(<a href=\"https://asbestosstar.egoism.jp/crash_detector/crash_detector_servidor.rb\">https://asbestosstar.egoism.jp/crash_detector/crash_detector_servidor.rb</a>)에서는 보고서를 처리하지 않으며, "
				+ "허용되지 않은 링크만 제거합니다. 소스 코드는 여기 있습니다: <a href=\"https://pagure.io/CrashDetectorMC/blob/main/f/src/main/resources/crash_detector_servidor.rb\">소스 코드</a>. "
				...
				+ "<a href=\"https://www.ppc.go.jp/files/pdf/20220401_personal_basicpolicy.pdf\">일본 개인정보 보호 기본 정책</a>.";
	}
	...
	public String errorConfigMCForge() {
		return "<b style='color:#" + config.obtenerColorError() + "'>" + "NightConfig/Forge 구성 치명적 오류: "
...
//Coreano
	...
	public String error_entrypoint_fabric_html(String modNombre) {
		return "<b style='color:#" + config.obtenerColorError() + "'>" + "<b>" + escapeHtml(modNombre)
				+ "</b> 모드를 로드하는 중 오류가 발생했습니다.</b>" + "<ul>" + "<li>해당 모드가 구성 요소(예: 설정 메뉴)를 초기화하는 데 실패했습니다.</li>"
				+ "<li>이는 일반적으로 Minecraft, Fabric 또는 다른 모드와의 버전 불일치로 인해 발생합니다.</li>" + "</ul>" + "<p>오류가 지속되면 <b>"
				+ escapeHtml(modNombre) + "</b> 모드를 삭제하거나 업데이트하세요.</p>";
	}
	...
	public String error_en_garde_html() {
		return "<b style='color:#" + config.obtenerColorError() + "'>" + "<b>En Garde!</b> 모드와 관련된 오류가 감지되었습니다.</b>"
				+ "<ul>" + "<li>이 모드는 근접 전투 메커니즘(패리, 블로킹 등)을 추가합니다.</li>"
				+ "<li>이 오류는 일반적으로 다른 전투 모드(Epic Fight, DualRiders 등)와의 호환성 문제나 Minecraft에 맞지 않는 버전 사용으로 발생합니다.</li>"
				+ "</ul>" + "<p>고급 전투를 사용하지 않는다면 충돌을 방지하기 위해 En Garde!를 제거하는 것을 고려해 보세요.</p>";
	}
	...
	public String error_idletweaks_html() {
		return "<b style='color:#" + config.obtenerColorError() + "'>" + "<b>IdleTweaks</b> 모드로 인한 오류가 감지되었습니다.</b>"
				+ "<ul>"
				+ "<li>IdleTweaks가 이미 존재하지 않는 네트워크 채널을 해제하려 시도했습니다 (<code>Tried to release unknown channel</code>).</li>"
				+ "<li>이 오류는 일반적으로 모드의 이전 버전이나 잘못 구성된 서버에서 발생합니다.</li>" + "</ul>"
	...
	public String lanzer_no_animado_titulo(String id) {
		return "<b style='color:#" + config.obtenerColorAdvertencia() + "'>" + "당신은 추천 목록에 없는 런처 <code>" + id
				+ "</code>를 사용하고 있습니다.</b>";
	}

	@Override
	public String lanzer_no_animado_problemas_comunes() {
		return "<p>작동할 수도 있지만, 비추천 런처는 흔히 다음을 일으킵니다:</p>" + "<ul>" + "<li>모드 또는 앱의 손상된 설치.</li>"
				+ "<li>게임이 시작되지 않거나 명확한 오류 없이 멈춤.</li>" + "<li>비정상적인 폴더 구조(진단을 어렵게 함).</li>"
				+ "<li>Java, 메모리 또는 모드에 대한 예측 불가한 동작.</li>" + "</ul>";
	}
	...
	public String lanzer_desanimado_problemas_comunes() {
		return "<p>권장하지 않는 런처는 다음을 야기할 수 있습니다:</p>" + "<ul>" + "<li>앱 또는 모드의 손상된 설치.</li>"
				+ "<li>게임이 시작되지 않거나 조용히 실패.</li>" + "<li>비정상적인 파일 구조(디버그 곤란).</li>"
				+ "<li>모드, Java, 메모리를 어떻게 관리하는지 불확실.</li>" + "</ul>";
	}
	...
	public String falta_mod_animado_titulo() {
		return "<b style='color:#" + config.obtenerColorError() + "'>" + "이 환경에 권장되는 모드가 누락되었습니다.</b>";
	}
	...
	public String tienes_mod_desanimado_titulo() {
		return "<b style='color:#" + config.obtenerColorError() + "'>" + "설치본에서 권장하지 않는 모드가 감지되었습니다.</b>";
	}
	...
	public String descripcionDerechosPirateria() {
		return "" + "반도용 경고를 활성화하기로 결정한 경우, 예방 조치로서 지원을 요청하는 사람의 권리를 여기에 정의하는 것이 좋습니다.\n\n"
				...
				+ "흔한 오해와 달리, 많은 인기 커뮤니티 및 지원 채널은 도움을 제공하기 위해 반도용 경고를 요구하지 않습니다. 그러나 "
				+ "사용자가 어쨌든 지원 채널에 접근할 경우 이러한 권리를 문서화하는 것이 유용할 수 있습니다.\n\n"
	...
	public String mensajeIndependenteFlywheel(Set<String> mods) {
		...
		String mensaje = "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "<p>현재 <b>Create</b> 버전은 <b>Flywheel을 이미 포함</b>하고 있으므로 별도로 설치하면 " + "호환성 충돌과 로딩 오류가 발생합니다.</p>"
				+ "<p> Flywheel에 명시적으로 의존하는 일부 모드는 " + "<b>작동하지 않거나</b> <b>불안정하게 작동</b>할 수 있습니다. "
				+ "특정 고급 상황에서는 <b><code>mods.toml</code> 파일을 수동으로 편집</b>하여 버전 범위를 조정하면 "
				+ "작동할 수도 있지만, 이는 <b>권장되지 않습니다</b>.</p>"
				+ (mods.isEmpty() ? ""
						: "<p><b>Flywheel을 참조하는 감지된 모드:</b></p>" + "<ul>" + listaMods.toString() + "</ul>")
	...
	public String mensajeFloralEnchantments() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "<i>Floral Enchantments</i> 모드와 관련된 오류가 감지되었습니다.</b>" + "<p>크래시는 게임 데이터를 처리하는 도중 모드 내부 오류로 인해 발생하며, "
				+ "실행 중 <b>NullPointerException</b>을 유발합니다.</p>" + "<p>이 문제는 일반적으로 모드를 업데이트하거나 제거하여 해결됩니다.</p>";
	}
	...
	public String mensajeIrisSombrasTerreno() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Iris에서 쉐이더(shaders)를 사용할 때 지형 그림자 오류가 감지되었습니다.</b>" + "<p>이 문제는 지형 렌더링 중 발생합니다.</p>"
				+ "<p><b>쉐이더 없이 게임을 실행해 보거나</b> 그래픽 품질을 낮추는 것이 좋습니다, " + "특히 <b>Ultra</b> 설정에서요.</p>";
	}
	...
	public String mensajeTickLargoServidor() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>" + "서버 틱이 지나치게 길게 감지되었습니다.</b>"
	...
	public String mensajeAdvertenciaIdiomaCoreano() {
		return "조선어 번역에는 남조선식 속어가 포함되어 있으며, 법을 준수하기 위해 이를 피해야 합니다. "
				+ "외국어, 특히 남쪽에서 유입된 언어의 사용은 「평양 문화어 보호법」에 따라 엄격히 금지됩니다.";
	}

	public String enlaceDocumentacionIdiomaCoreano() {
		return "자세한 내용은 법령 공식 문서를 참조하십시오: "
				+ "<a href='https://www.dailynk.com/english/wp-content/uploads/sites/2/2023/03/Pyongyang-Cultural-Language-Protection-Act_English-and-Korean-Versions_Daily-NK.pdf'>평양 문화어 보호법</a>";
	}
	...
	public String mensajeSimpleCloudsIncompatibilidadShaders() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "<p><b>권장 선택사항:</b></p>" + "<ul>" + "<li>셰이더를 사용하려면 <b>Simple Clouds</b>를 삭제하십시오.</li>"
				+ "<li>아니면 Simple Clouds를 유지하려면 <b>Iris 또는 Oculus</b>를 제거하십시오.</li>" + "</ul>"
	...
	public String malware_brightsdk_detectado() {
		return "잠재적 Bright SDK가 감지되었습니다. 증거:";
	}
	...
	public String mensajeOculusIrisUnknownShaderVariable() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "이는 주로 <b>Oculus</b>, <b>Iris</b> 또는 사용 중인 <b>쉐이더 팩</b>과 관련이 있습니다.</p>" + "<p><b>권장되는 시험 순서：</b></p>"
				+ "<ul>" + "<li>먼저，<b>쉐이더를 활성화하지 않고</b> 게임을 시작하십시오.</li>"
				+ "<li>문제가 계속되면 <b>Oculus 또는 Iris 없이</b> 시작해 보십시오.</li>"
				+ "<li><b>Oculus/Iris</b>, <b>쉐이더 팩</b> 및 관련 그래픽 모드를 최신 버전으로 갱신하십시오.</li>"
				+ "<li>다른 렌더링 또는 그래픽 모드를 사용하는 경우 상호 간 호환성 문제를 검토하십시오.</li>" + "</ul>"
	...
	public String mensajeItemNoExiste(String itemFaltante, String namespace) {
		...
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "존재하지 않는 항목을 사용하려고 했습니다.</b>" + "<p>기록에는 <b>Item: " + itemHtml
				+ " does not exist</b> 줄이 포함되어 있습니다.</p>" + "<p>이는 일반적으로 일부 <b>데이터팩</b>, <b>모드</b> 또는 <b>설정</b>이 "
				+ "게임에 없는 항목을 참조하고 있음을 의미합니다.</p>" + "<p><b>확인할 사항：</b></p>" + "<ul>" + "<li>항목 <b>" + itemHtml
				+ "</b>을 제공해야 하는 모드가 설치되어 있는지 확인하십시오.</li>" + "<li>설치되어 있다면 <b>잘못된 버전</b>이거나 항목이 변경되거나 삭제되었을 수 있으며，"
				+ "모드에 문제가 있어 제거하는 것이 좋을 수 있습니다.</li>" + "<li>해당 모드가 없다면 <b>설치</b>해 보십시오.</li>" + "</ul>"
				+ "<p><b>어떤 모드나 데이터팩이 해당 항목을 요청하는지 알아내는 방법：</b></p>" + "<ul>"
				+ "<li>측면 바에서 <b>grepr</b> 유틸리티를 사용하십시오.</li>" + "<li>폴더를 선택하지 <b>마십시오</b>.</li>"
				+ "<li><b>search in archives</b> 옵션을 활성화하십시오.</li>"
				+ "<li>검색 텍스트에 <b>namespace</b>，즉 콜론 앞의 부분을 입력하십시오： " + "<b>" + namespaceHtml + "</b>.</li>" + "</ul>"
	...
	public String mensajeCobblemonPinkanIslandsRhyhornModelo() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "<li>문제 중 하나의 모드를 갱신한 후에 시작되었다면，다른 버전 조합을 시험해 보십시오.</li>" + "</ul>"
	...
	public String mensajeColdSweatInitDynamicTags() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Cold Sweat에서 오류가 발견되였습니다.</b>" + "<p>기록에는 <b>$cold_sweat$onBuildStart</b>, "
				+ "<b>InitDynamicTagsEvent.fillTag</b> 및 <b>NullPointerException</b>과 같은 징후가 포함되어 있으며, "
				+ "등록부가 null로 나타납니다.</p>" + "<p>이는 일반적으로 <b>Cold Sweat</b>가 "
				+ "<b>동적 태그(dynamic tags)</b>를 구축하거나 채우는 과정에서 문제가 발생했음을 나타내며, " + "보통 호환성 문제, 모드 내부 오류, "
				+ "또는 다른 모드나 데이터팩과의 충돌 조합 때문입니다.</p>" + "<p><b>시험해 볼 사항：</b></p>" + "<ul>"
				...
				+ "<li>오류가 모드를 갱신한 후에 시작되었다면, 다른 버전 조합을 시험해 보십시오.</li>" + "</ul>"
	...
	public String mensajeClassCastExceptionGeneral(String lineaClassCast) {
		String detalle = lineaClassCast == null || lineaClassCast.isEmpty() ? ""
				: "<p><b>발견된 줄：</b></p><p><code>" + lineaClassCast + "</code></p>";

		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "ClassCastException이 발견되였습니다.</b>" + "<p>이는 어떤 클래스가 호환되지 않는 다른 클래스나 인터페이스인 것처럼 처리되였음을 의미합니다.</p>"
				+ detalle + "<p>이러한 유형의 오류는 보통 다음 상황 중 하나로 인해 발생합니다：</p>" + "<ul>" + "<li>서로 <b>호환되지 않는 두 모드</b>.</li>"
				+ "<li>클래스를 수정하여 게임의 다른 부분이 다른 유형을 기대하게 만드는 <b>Mixins</b>, <b>transformers</b> 또는 패치.</li>"
				+ "<li>잘못된 형변환(miscast)을 유발하는 <b>stacktrace</b>에 존재하는 다른 모드.</li>" + "</ul>" + "<p><b>확인할 사항：</b></p>"
				+ "<ul>" + "<li>이 오류와 관련된 <b>stacktrace</b> 줄을 살펴보십시오.</li>"
				+ "<li><b>$modid$algo</b> 형식의 모드 또는 클래스 이름에 특별히 주의하십시오. 이는 일반적으로 관련된 모드를 가리키기 때문입니다.</li>"
				+ "<li>잘못된 변환과 관련되어 보이는 모드를 갱신, 제거 또는 분리해 보십시오.</li>" + "</ul>"
				+ "<p><b>ClassCastException</b>이 항상 치명적인 것은 아니지만, 매우 자주 그렇습니다.</p>";
	}

	@Override
	public String nombreClassCastExceptionGeneral() {
		return "ClassCastException 발견됨";
	}

	@Override
	public String mensajeValkyrienSkiesTournamentLithiumPoiInjection() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Valkyrien Skies Tournament와 Lithium/Radium 사이에 호환되지 않을 가능성이 발견되였습니다.</b>"
				+ "<p>기록에는 <b>InvalidInjectionException</b>이 포함되어 있으며, 여기에는 "
				+ "<b>Lithium</b>의 <b>POI</b> 관련 mixin과 <b>valkyrienskies-common.mixins.json:feature.poi.MixinPOIManager</b>이 함께 나타납니다.</p>"
				+ "<p>이 문제는 <b>구버전 Lithium</b> 또는 <b>구버전 Lithium 기반 fork</b>"
				+ "(예: <b>Radium Reforged</b>)를 <b>VS Tournament</b>와 함께 사용할 때 흔히 발생합니다.</p>"
				+ "<p><b>시험해 볼 사항：</b></p>" + "<ul>" + "<li><b>Lithium</b>를 더 새롭고 호환되는 버전으로 갱신하십시오.</li>"
				+ "<li><b>Forge/NeoForge</b> 환경에서 <b>Radium Reforged</b>나 다른 구버전 fork를 사용 중이라면 제거하십시오.</li>"
				+ "<li>대신 <b>Harium</b>을 시험해 보십시오. 이는 Radium의 현대적인 fork로, Lithium의 최근 개선 사항과 동기화되어 있습니다.</li>"
				+ "<li>모드를 갱신한 후 문제가 시작되었다면, <b>VS Tournament</b>와 AI/POI 최적화 모드 간의 정확한 조합을 검토하십시오.</li>" + "</ul>"
				+ "<p>실제로 이 장애는 일반적으로 <b>VS Tournament</b>와 잘 맞지 않는 <b>Lithium/Radium</b>의 구버전 구현에서 비롯됩니다.</p>";
	}

	@Override
	public String nombreValkyrienSkiesTournamentLithiumPoiInjection() {
		return "VS Tournament와 Lithium/Radium의 호환성 문제";
	}

	@Override
	public String mensajeVSTournamentVSConfigClassNoExiste() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "VS Tournament가 사용 중인 Valkyrien Skies 버전에 비해 너무 오래된 것으로 보입니다.</b>"
				+ "<p>기록에는 <b>NoClassDefFoundError</b>가 포함되어 있으며, "
				+ "<b>org/valkyrienskies/core/impl/config/VSConfigClass</b> 관련 오류와 함께 "
				+ "<b>org.valkyrienskies.tournament.TournamentMod.init(...)</b> 줄이 나타납니다.</p>"
				+ "<p>이는 일반적으로 <b>VS Tournament의 오래된 버전</b>을 사용 중이며, "
				+ "더 이상 존재하지 않는 <b>Valkyrien Skies</b>의 내부 클래스를 호출하려고 시도했음을 의미합니다.</p>" + "<p><b>해야 할 일：</b></p>"
				+ "<ul>" + "<li>오래된 <b>VS Tournament</b>를 제거하십시오.</li>"
				+ "<li>대신 <b>VS Tournament Reforged</b>를 사용하십시오.</li>"
				+ "<li><b>Valkyrien Skies</b> 버전이 애드온이 지원하는 버전과 일치하는지 확인하십시오.</li>" + "</ul>"
				+ "<p><b>VS Tournament Reforged</b>로 변경을 권장하는 것은 프로젝트의 현재 상태와 부합합니다："
				+ "Tournament의 원래 버전은 여전히 1.18.2용 구버전 모드로 등재되어 있는 반면, "
				+ "<b>VS Tournament Reforged</b>는 별도로 배포되며 현재 Forge 1.20.1에서 Valkyrien "
				+ "2.4.9+를 지원한다고 공식 발표되어 있습니다.</p>";
	}
	...
	public String mensajeControlifyRemoveReloadingScreen() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "Controlify는 시스템이 완전히 준비되기 전에 구성을 초기화하려고 시도합니다.</p>" + "<p><b>권장 사항：</b></p>" + "<ul>"
				...
				+ "<li>문제가 계속되면 <b>Controlify</b>를 유지하고 로딩 화면을 수정하는 모든 모드를 제거하십시오.</li>" + "</ul>"
				+ "<p>로딩 화면을 수정하는 모드는 종종 다른 모드와 호환되지 않는 문제를 일으키며, " + "발생시킬 수 있는 문제에 비해 실제적인 이점은 거의 없습니다.</p>";
	}
	...
	public String mensajeBiomesOPlentyFogDataLiquidosPersonalizados() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "<p><b>권장 사항：</b></p>" + "<ul>" + "<li>플레이어 데이터를 편집하여 세계의 다른 위치로 이동해 보십시오.</li>"
				+ "<li><b>Biomes O' Plenty</b> 없이 세계를 로드해 보십시오.</li>" + "<li>플레이어를 이동한 후 세계가 로드된다면, 문제는 특정 지역, "
				+ "특정 생물 군계 또는 근처의 사용자 정의 액체에서 발생할 가능성이 높습니다.</li>" + "<li><b>Biomes O' Plenty</b>와 렌더링, 안개, "
				+ "쉐이더(shaders) 또는 차원과 관련된 모드를 갱신해 볼 수도 있습니다.</li>" + "</ul>"
				+ "<p>Biomes O' Plenty를 제거하면 게임이 시작된다면, 플레이어가 해당 모드가 추가한 생물 군계 " + "또는 유체 내부나 근처에 있었는지 확인하십시오.</p>";
	}
	...
	public String mensajeKotlinReflectionInternalErrorVersion() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "이 경우 <b>Inventory Profiles Next</b>의 클래스가 나타나지만, 동일한 문제는 Kotlin을 사용하는 " + "다른 모드에서도 발생할 수 있습니다.</p>"
				+ "<p><b>권장 사항：</b></p>" + "<ul>"
				...
				+ "<b>Fabric Language Kotlin</b>의 버전을 변경해 보십시오.</li>" + "</ul>" + "<p>관련 기술 참조: "
	...
	public String mensajeMotionBlurBufferCerrado() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Motion Blur와 관련된 잠재적 문제.</b>"
				+ "<p>로그에 <b>net.natural.motionblur.shader.FrameBlendingManager.writeBlendParamsUBO</b>에 대한 참조와 "
				+ "<b>java.lang.IllegalStateException: Buffer already closed</b> 오류가 포함되어 있습니다.</p>"
				+ "<p>이 줄들은 로그에서 분리되어 나타날 수 있지만, 함께 나타나면 일반적으로 문제가 <b>Motion Blur</b> mod "
				+ "또는 그래픽 shader/buffer 처리와 관련이 있음을 나타냅니다.</p>" + "<p><b>권장 옵션:</b></p>" + "<ul>"
				+ "<li><b>Motion Blur</b> 없이 게임을 시작해 보세요.</li>"
				+ "<li>해당 mod 없이 게임이 정상적으로 시작되면, 제거된 상태로 유지하거나 최신 버전을 찾으세요.</li>"
				+ "<li>문제가 계속되면 shaders 또는 기타 렌더링 mods 없이 시도해 볼 수도 있습니다.</li>" + "</ul>";
	}

	@Override
	public String nombreMotionBlurBufferCerrado() {
		return "잠재적 문제: Motion Blur";
	}
	...
	public String mensajeGeneratorAcceleratorOwoVersion() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Generator Accelerator와 잠재적 충돌.</b>" + "<p>로그에는 <b>Found</b> 및 <b>Available</b> 서명 간의 차이와 "
				+ "<b>Generator Accelerator</b>의 클래스(예: <b>dev/sixik/generator_accelerator/common/features/FastTarget</b>)가 포함되어 있습니다.</p>"
				+ "<p>이 오류는 아마도 <b>Generator Accelerator</b> 때문에 발생했을 것입니다. 또한 해당 mod와 특정 버전의 <b>owo-lib</b> 간의 호환성 문제와 관련될 수 있습니다.</p>"
				+ "<p><b>권장 옵션:</b></p>" + "<ul>" + "<li><b>Generator Accelerator</b> 없이 게임을 시작해 보세요.</li>"
				+ "<li>게임이 정상적으로 시작되면 해당 mod를 제거하거나 다른 버전을 찾으세요.</li>"
				+ "<li>다른 mod들도 owo에 의존하는 경우 <b>owo-lib</b>의 버전을 업데이트하거나 변경해 보세요.</li>"
				+ "<li><b>Generator Accelerator</b>, <b>owo-lib</b>, loader 및 Minecraft 버전이 서로 호환되는지 확인하세요.</li>"
				+ "</ul>"
				+ "<p>가장 유력한 원인은 Generator Accelerator가 현재 클래스 또는 종속성 버전과 일치하지 않는 서명으로 수정을 적용하려고 시도했기 때문입니다.</p>";
	}

	@Override
	public String nombreGeneratorAcceleratorOwoVersion() {
		return "잠재적 충돌: Generator Accelerator 및 owo-lib";
	}
	...
	public String mensajeFabricRenderingApiFaltaIndium() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				+ "Fabric Rendering API와 호환되는 렌더러가 없습니다.</b>"
				+ "<p>로그에는 <b>RendererAccess.getRenderer()</b>이(가) <b>null</b>을 반환하는 오류가 포함되어 있으며, "
				+ " 이로 인해 <b>Renderer.materialFinder()</b> 사용 시 실패합니다.</p>"
				+ "<p>이 문제는 일반적으로 <b>Fabric Rendering API</b>가 제대로 사용할 수 없을 때 발생합니다. "
				+ "흔한 원인은 <b>Sodium</b> 사용이며, 특히 다른 mods가 예상하는 렌더링 시스템의 일부를 대체하거나 비활성화하는 구버전일 경우입니다.</p>"
				+ "<p><b>권장 해결 방법:</b></p>" + "<ul>" + "<li>mod <b>Indium</b>을 설치하세요.</li>"
				+ "<li><b>Indium</b>이 사용 중인 <b>Sodium</b>, Fabric Loader 및 Minecraft 버전과 호환되는지 확인하세요.</li>"
				+ "<li>이미 Indium이 설치되어 있다면 <b>Sodium</b>, <b>Indium</b> 및 <b>Fabric API</b>를 업데이트하세요.</li>"
				+ "<li>문제가 계속되면 렌더러와 관련된 문제인지 확인하기 위해 일시적으로 Sodium 없이 테스트해 보세요.</li>" + "</ul>"
				+ "<p>Indium은 일반적으로 Sodium이 설치된 상태에서 해당 시스템에 의존하는 mods를 위해 Fabric Rendering API와의 호환성을 복원합니다.</p>";
	}
	...
	public String mensajeEntradaDuplicadaIdModerno() {
		return "<b style='color:#" + Config.obtenerInstancia().obtenerColorError() + "'>"
				...
				+ "<b>current=maroon, previous=mint</b>입니다.</p>" + "<p>최신 버전의 Minecraft에서는 두 개의 mod가 동일한 내부 ID를 사용하여 "
				+ "서로 다른 항목을 등록하려고 할 때 이러한 유형의 오류가 자주 발생합니다.</p>" + "<p><b>권장 옵션:</b></p>" + "<ul>"
				...
				+ "<li>일부 이름이나 ID는 컴파일된 클래스 내에 있을 수 있으므로 <b>바이너리 파일</b> 검색도 활성화하세요.</li>" + "</ul>"
	...
	public String ideScriptProyectoDeshabilitadoAviso() {
		return "이 유형의 프로젝트는 GUI에 이미 존재하지만 언어 서버 또는 분석기가 추가될 때까지 비활성화되어 있습니다.";
	}
	...
	public String ideScriptConfirmarDescargaDeps(int cantidad, String lista) {
		return "스크립트 IDE를 위해 " + cantidad + "개의 종속성이 다운로드됩니다:\n\n" + lista + "\n그 후 클래스패스에 적용되려면 재시작이 필요할 수 있습니다.";
	}
	...
	public String ideScriptProyectoDatapackResourcepack() {
		return "Minecraft 데이터팩 / 리소스팩 (JSON)";
	}
	...
	public String nombreVerificacionMeltdown() {
		return TraduccionesSeguridadCPU.texto("ko", "check_meltdown");
	}

	@Override
	public String nombreVerificacionSpectre() {
		return TraduccionesSeguridadCPU.texto("ko", "check_spectre");
	}

	@Override
	public String desactivarVerificacionMeltdown() {
		return TraduccionesSeguridadCPU.texto("ko", "disable_meltdown");
	}

	@Override
	public String desactivarVerificacionSpectre() {
		return TraduccionesSeguridadCPU.texto("ko", "disable_spectre");
	}

	@Override
	public String abrirDocumentacionOficialSeguridadCPU() {
		return TraduccionesSeguridadCPU.texto("ko", "open_docs");
	}

	@Override
	public String errorComprobacionSeguridadCPU(String verificacion, String detalle) {
		return TraduccionesSeguridadCPU.texto("ko", "check_error", verificacion, detalle);
	}
	...
	public String tituloProteccionProcesador(String vulnerabilidad) {
		return TraduccionesSeguridadCPU.texto("ko", "title_protection", vulnerabilidad);
	}

	@Override
	public String etiquetaSeguridadCpuCPU() {
		return TraduccionesSeguridadCPU.texto("ko", "label_cpu");
	}

	@Override
	public String etiquetaSeguridadCpuFabricante() {
		return TraduccionesSeguridadCPU.texto("ko", "label_vendor");
	}

	@Override
	public String etiquetaSeguridadCpuArquitectura() {
		return TraduccionesSeguridadCPU.texto("ko", "label_arch");
	}

	@Override
	public String etiquetaSeguridadCpuSistemaOperativo() {
		return TraduccionesSeguridadCPU.texto("ko", "label_os");
	}

	@Override
	public String etiquetaSeguridadCpuAfectado() {
		return TraduccionesSeguridadCPU.texto("ko", "label_affected");
	}

	@Override
	public String etiquetaSeguridadCpuEstadoGeneral() {
		return TraduccionesSeguridadCPU.texto("ko", "label_overall");
	}

	@Override
	public String etiquetaSeguridadCpuMitigacionSO() {
		return TraduccionesSeguridadCPU.texto("ko", "label_os_mitigation");
	}

	@Override
	public String etiquetaSeguridadCpuMicrocodigoFirmware() {
		return TraduccionesSeguridadCPU.texto("ko", "label_microcode");
	}

	@Override
	public String etiquetaSeguridadCpuRevisionDetectada() {
		return TraduccionesSeguridadCPU.texto("ko", "label_revision");
	}

	@Override
	public String etiquetaSeguridadCpuNota() {
		return TraduccionesSeguridadCPU.texto("ko", "label_note");
	}

	@Override
	public String etiquetaSeguridadCpuEvidencia() {
		return TraduccionesSeguridadCPU.texto("ko", "label_evidence");
	}

	@Override
	public String etiquetaSeguridadCpuAccionRecomendada() {
		return TraduccionesSeguridadCPU.texto("ko", "label_action");
	}

	@Override
	public String estadoAfectacionSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoAfectacion("ko", estado);
	}

	@Override
	public String estadoMitigacionSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoMitigacion("ko", estado);
	}

	@Override
	public String estadoComponenteSeguridadCPU(String estado) {
		return TraduccionesSeguridadCPU.estadoComponente("ko", estado);
	}

	@Override
	public String notaSeguridadCpuRegistroRemoto() {
		return TraduccionesSeguridadCPU.texto("ko", "remote_note");
	}

	@Override
	public String mensajeSeguridadCPU(String clave, String... argumentos) {
		return TraduccionesSeguridadCPU.mensaje("ko", clave, argumentos);
	}
