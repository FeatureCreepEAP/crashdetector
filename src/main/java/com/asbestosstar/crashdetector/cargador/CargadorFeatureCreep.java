...
public class CargadorFeatureCreep implements Cargador{
	...
	public static List<String> parsearNombreModuloJBoss(byte[] contenido) throws IOException {
	    ...
	    if (pos < 0) return java.util.Collections.emptyList();
	    int inicio = pos + clave.length();
	    int fin = xml.indexOf("\"", inicio);
	    if (fin < 0) return java.util.Collections.emptyList();
	    String nombre = xml.substring(inicio, fin).trim();
	    if (nombre.isEmpty()) return java.util.Collections.emptyList();
	...
	/**
	 * Extrae el nombre del mod de un archivo HOI4 con extension .mod https://hoi4.paradoxwikis.com/Mod_structure
	 */
