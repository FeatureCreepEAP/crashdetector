 ...
 *
 * OBJETIVO: concentrar aquí la lógica técnica (estructura base, wiring,
 * SwingWorker) y exponer hooks de apariencia para que la implementación
 * concrete colores, fuentes e imagen decorativa, además de textos NO
 * localizados (por ejemplo, estados).
 *
...
public abstract class EscanerMCreatorGUI extends JFrame implements CrashDetectorGUI, BotonDeBarraLateralDerecha {
	...
	/**
	 * Debe restablecer colores, bordes y cualquier texto NO localizado (p.ej.
	 * título ventana/estados).
	 */
	protected void aplicarApariencia() {
		// Fondo ventana y paneles
		getContentPane().setBackground(colorFondoVentana());
		if (panelContenido != null)
			panelContenido.setBackground(colorFondoVentana());
		if (panelContenidoConImagen != null)
			panelContenidoConImagen.setBackground(colorFondoVentana());
