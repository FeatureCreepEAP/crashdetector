...
/**
 * Implementación VShojo centrada en APARIENCIA. - Lee la paleta desde Config
 * (convertida a Color) - Restablece estilos/colores en recargarApariencia() -
 * En macOS fuerza opaco para que se pinten los colores
 *
 * Llamado recomendado: NoRegistroDeLauncherVShojo d = new
 * NoRegistroDeLauncherVShojo(); d.preparar(ownerFrame, instant); // build +
 * theme (ANTES de init) d.init(); // mostrar
 */
public class NoRegistroDeLauncherVShojo extends NoRegistroLanzadorGUI {
	...
	protected void aplicarApariencia() {
		...
		if (raizPanel != null)
			raizPanel.setBackground(COLOR_FONDO);
		if (encabezadoPanel != null)
			encabezadoPanel.setBackground(COLOR_FONDO);
		if (centroPanel != null)
			centroPanel.setBackground(COLOR_FONDO);
		if (panelImagenYBoton != null)
			panelImagenYBoton.setBackground(COLOR_FONDO);
		if (piePanel != null)
			piePanel.setBackground(COLOR_FONDO);
	...
	@Override
	protected Color colorFondoBase() {
		return COLOR_FONDO;
	}

	@Override
	protected Color colorTextoBase() {
		return COLOR_TEXTO;
	}

	@Override
	protected Color colorBotonBase() {
		return COLOR_BOTON;
	}

	@Override
	protected Color colorCajaTextoBase() {
		return COLOR_CAJA;
	}

	@Override
	protected Color colorEnlaceBase() {
		return COLOR_ENLACE;
	}
	...
	protected void estilizarCombo(javax.swing.JComboBox<?> combo) {
		...
		if (!com.asbestosstar.crashdetector.gui.CrashDetectorGUI.esMac()
			...
			((com.asbestosstar.crashdetector.gui.ComboIdiomasConIcono) comboBoxIdioma).aplicarColores(COLOR_BOTON,
					COLOR_TEXTO);
		}
	}
}
