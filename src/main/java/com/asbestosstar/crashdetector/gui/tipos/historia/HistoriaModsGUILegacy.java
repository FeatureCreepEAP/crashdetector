 ...
 * 
 * - Define paleta de colores, fuentes, paddings, tamaños preferidos.
 * - Inserta imagen "clio.png" y un panel descriptivo (texto NO localizado permitido aquí).
 * - Restablece todo en recargarApariencia().
 */
public class HistoriaModsGUILegacy extends HistoriaDeModsGUI {
    ...
    @Override
    protected Color colorEstadoExito() { return COLOR_EXITO; }

    @Override
    protected Color colorEstadoFallo() { return COLOR_FALLO; }

    @Override
    protected String colorHexParaResultadoAnadido() { return HEX_PLUS; }

    @Override
    protected String colorHexParaResultadoEliminado() { return HEX_MINUS; }
    ...
    /**
     * Restablece por completo colores, fuentes, bordes, paddings y textos NO localizados
     * (p.ej. explicación del panel inferior). No toca datos ni selección.
     */
    @Override
    protected void aplicarApariencia() {
        ...
        if (scrollIzquierdo != null) scrollIzquierdo.setBorder(BorderFactory.createLineBorder(new Color(0xDDDDDD)));
        if (scrollDerecho  != null)  scrollDerecho.setBorder(BorderFactory.createLineBorder(new Color(0xDDDDDD)));
        if (scrollResultado!= null)  scrollResultado.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));
    ...
    /**
     * Crea/reemplaza el panel inferior puramente de apariencia (no datos),
     * con un bloque HTML de ayuda NO localizado y la imagen clio.png.
     */
    private void instalarPanelInferiorConImagenYTexto() {
        ...
        String ayudaNoLocalizada = String.format(
            "<html>" +
            ...
            "  <span style='color:%s;'>eliminados (&#8722;)</span> basados en nombres normalizados." + // &#8722; = signo menos (compat)
            "</body>" +
            "</html>", HEX_PLUS, HEX_MINUS
        );
    ...
    // Si tu interfaz BotonDeBarraLateralDerecha exige etiquetaDelBoton(), puedes descomentar:
    // @Override
