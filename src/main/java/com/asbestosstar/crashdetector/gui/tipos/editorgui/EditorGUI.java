...
/**
 * Clase base abstracta para el editor de GUIs. Contiene la lógica para
 * seleccionar tipos de GUI y sus instancias. La apariencia y el layout se
 * definen en implementaciones concretas.
 */
	...
	protected java.util.List<com.asbestosstar.crashdetector.config.ElementoConfig> obtenerElementosConfigSeleccionados() {
		...
		if (tipo != null) {
			...
			if (supplier != null) {
				...
				if (gui != null) {
					try {
						java.util.List<com.asbestosstar.crashdetector.config.ElementoConfig> elementos = gui
