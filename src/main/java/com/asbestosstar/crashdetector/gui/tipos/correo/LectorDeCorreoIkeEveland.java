...
public class LectorDeCorreoIkeEveland extends LectorDeCorreoGUI {
	...
	private JPanel crearPanelMensajes() {
		...
		modeloMensajes = new DefaultTableModel(
				new Object[] { MonitorDePID.idioma.correoColumnaEstado(), MonitorDePID.idioma.correoColumnaDe(),
						MonitorDePID.idioma.correoColumnaAsunto(), MonitorDePID.idioma.correoColumnaFecha() },
				0) {
			private static final long serialVersionUID = 1L;
	...
	protected CuentaCorreo pedirCuenta(CuentaCorreo existente) {
		...
		JTextField puerto = new JTextField(existente == null ? Integer.toString(PUERTO_IMAPS_PREDETERMINADO)
				: Integer.toString(existente.puerto()), 8);
		...
		int respuesta = JOptionPane.showConfirmDialog(this, panel,
				existente == null ? MonitorDePID.idioma.correoAgregarCuenta()
						: MonitorDePID.idioma.correoEditarCuenta(),
	...
	protected boolean confirmarEliminarCuenta(CuentaCorreo cuenta) {
		int respuesta = JOptionPane.showConfirmDialog(this,
				MonitorDePID.idioma.correoConfirmarEliminar(cuenta.nombre()),
				MonitorDePID.idioma.correoEliminarCuenta(), JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
