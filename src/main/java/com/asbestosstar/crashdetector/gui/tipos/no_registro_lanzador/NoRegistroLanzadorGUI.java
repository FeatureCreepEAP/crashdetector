...
/**
 * Diálogo abstracto cuando el launcher no provee registro.
 *
 * - preparar(owner, instant): construye la UI, aplica apariencia y ata el cierre de ventana a Omitir.
 * - init(): visible(true)
 * - Subclase define paleta y aplica estilo en aplicarApariencia().
 */
public abstract class NoRegistroLanzadorGUI extends JDialog implements CrashDetectorGUI {

    private static final long serialVersionUID = 1L;

    /** Mantener registro de GUIs (no eliminar). */
    public static Map<String, Supplier<NoRegistroLanzadorGUI>> GUIS = new HashMap<>();

    // Opciones de selector (etiquetas mostradas)
    protected static final String GEN = "Genérico";
    protected static final String CURSE = "CurseForge";
    protected static final String PRISM = "Prism/MultiMC+++";
    protected static final String HMCL = "HMCL";
    protected static final String FENIX = "Fénix";
    protected static final String ATL = "ATLauncher";
    protected static final String GD = "GDLauncher";
    protected static final String BATTLY = "Battly Launcher";
    protected static final String NIGHTWORLD = "Nightworld";
    protected static final String MCSERVER = "Servidor de Minecraft";
    protected static final String ENLACE_MD = "Enlace MD";

    // Escalado imagen de ayuda (técnico)
    protected static final int MAX_ANCHO_IMAGEN = 500;
    protected static final int MAX_ALTO_CASI_CUADRADA = 220;
    protected static final int MAX_ALTO_PANORAMICA = 120;
    protected static final int ALTURA_MINIMA_SIN_IMAGEN = 120;

    protected boolean building = false;

    // Controles (técnico)
    protected JComboBox<String> selector;
    protected JComboBox<String> comboBoxIdioma;

    protected final JLabel imagenLbl = new JLabel("", SwingConstants.CENTER);
    protected final JLabel vshojoLbl = new JLabel("", SwingConstants.CENTER);
    protected final JButton seleccionarCarpetaBtn = new JButton(MonitorDePID.idioma.seleccionarCarpeta());

    protected JEditorPane descripcionHtml;
    protected JScrollPane descScroll;

    protected TextArea areaTexto;

    protected JButton botonGuardar;
    protected JButton botonOmitir;
    protected JButton botonProxy;

    protected JPanel panelAreaTexto;
    protected JPanel panelBajoImagen;

    // **** Paneles estructurales promovidos a campos (para aplicar tema correctamente) ****
    protected JPanel raizPanel;
    protected JPanel encabezadoPanel;
    protected JPanel centroPanel;
    protected JPanel panelImagenYBoton;
    protected JPanel piePanel;
    protected JPanel botonesPanel;
    ...
    protected JPanel construirContenido() {
        building = true;
        ...
        // Descripción HTML (raw HTML permitido)
        ...
        if (!CrashDetectorGUI.esMac()) {
            ((ComboIdiomasConIcono) comboBoxIdioma).aplicarColores(colorBotonBase(), colorTextoBase());
        }

        // Selección por idioma actual
        String code = MonitorDePID.idioma.codigo();
        if      ("es".equals(code)) comboBoxIdioma.setSelectedItem("Español");
        else if ("en".equals(code)) comboBoxIdioma.setSelectedItem("English");
        else if ("ar".equals(code)) comboBoxIdioma.setSelectedItem("العربية");
        else if ("pt".equals(code)) comboBoxIdioma.setSelectedItem("Português");
        else if ("fa".equals(code)) comboBoxIdioma.setSelectedItem("فارسی");
        else if ("ru".equals(code)) comboBoxIdioma.setSelectedItem("Русский");
        else if ("zh".equals(code)) comboBoxIdioma.setSelectedItem("简体中文");
        else if ("eo".equals(code)) comboBoxIdioma.setSelectedItem("Esperanto");
        else if ("ja".equals(code)) comboBoxIdioma.setSelectedItem("日本語");
        else if ("ko".equals(code)) comboBoxIdioma.setSelectedItem("한국어");
        else comboBoxIdioma.setSelectedItem("Español");
        ...
        filaControles.setBackground(this.colorFondoBase());
        ...
        botonGuardar.addActionListener(new java.awt.event.ActionListener() {
            @Override public void actionPerformed(java.awt.event.ActionEvent e) { guardarRegistros(); }
        });
        ...
        g0.gridx = 0; g0.gridy = 0; g0.anchor = GridBagConstraints.WEST;
        g0.insets = new java.awt.Insets(0, 0, 0, 8);
        piePanel.add(vshojoLbl, g0);

        GridBagConstraints g1 = new GridBagConstraints();
        g1.gridx = 1; g1.gridy = 0; g1.weightx = 1.0;
        g1.anchor = GridBagConstraints.EAST; g1.fill = GridBagConstraints.HORIZONTAL;
    ...
    protected void refrescarInterfaz() {
    ...
    protected String detectarPorDirectorio() {
        String rutaAbs = new File(System.getProperty("user.dir")).getAbsolutePath().toLowerCase();

        if (contiene(rutaAbs, "prismlauncher", "multimc", "polymc", "pollymc", "freesm", "fjord", "ultim", "prism")) return PRISM;
        if (contiene(rutaAbs, "atlauncher"))  return ATL;
        if (contiene(rutaAbs, "gdlauncher"))  return GD;
        if (contiene(rutaAbs, "curseforge", "overwolf")) return CURSE;
        if (contiene(rutaAbs, "hmcl"))        return HMCL;
        if (contiene(rutaAbs, "battly"))      return BATTLY;
        if (contiene(rutaAbs, "server"))      return MCSERVER;

        File f = new File(rutaAbs);
        for (int i = 0; i < 5 && f != null; i++, f = f.getParentFile()) {
            String nombre = f.getName().toLowerCase();
            if (contiene(nombre, "prismlauncher", "multimc", "polymc", "pollymc", "freesm", "fjord", "ultim", "prism")) return PRISM;
            if (nombre.contains("atlauncher")) return ATL;
            if (nombre.contains("gdlauncher")) return GD;
            if (contiene(nombre, "curseforge", "overwolf")) return CURSE;
            if (nombre.contains("hmcl")) return HMCL;
            if (nombre.contains("battly")) return BATTLY;
            if (nombre.contains("server")) return MCSERVER;
        }
        return GEN;
    }

    protected boolean contiene(String s, String... tokens) {
        for (int i = 0; i < tokens.length; i++) if (s.contains(tokens[i])) return true;
        return false;
    }

    // Acciones

    protected void abrirSelectorCarpeta() {
    ...
    protected void guardarRegistros() {
        ...
        if (areaTexto == null) { dispose(); return; }
        ...
        Thread writerThread = new Thread(new Runnable() {
            @Override public void run() {
                try (FileOutputStream fos = new FileOutputStream(cd_launcherlog); FileChannel ch = fos.getChannel()) {
                    ...
                    JOptionPane.showMessageDialog(NoRegistroLanzadorGUI.this, "Error: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        });

        writerThread.start();
        try { writerThread.join(timeoutSeconds * 1000); } catch (InterruptedException e) { Thread.currentThread().interrupt(); }
        dispose();
    }

    /** Overlay modal con GIF o barra indeterminada; cierra diálogo al terminar. */
    protected void ejecutarConOverlayDescarga(final Runnable tarea, String mensaje) {
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            @Override protected Void doInBackground() {
                try { tarea.run(); }
                    ...
                    JOptionPane.showMessageDialog(NoRegistroLanzadorGUI.this, "Error: " + t.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                }
                return null;
            }
    ...
    // ====== Hooks de apariencia (la implementación concreta los define) ======

    /** Debe restablecer todos los colores, bordes, paddings y textos NO localizados. */
    protected abstract void aplicarApariencia();

    protected abstract Color colorFondoBase();
    protected abstract Color colorTextoBase();
    protected abstract Color colorBotonBase();
    protected abstract Color colorCajaTextoBase();
    protected abstract Color colorEnlaceBase();

    protected String overlayMensaje() { return "Descargando y preparando enlaces..."; }
    protected String gifDescargaPath() { return "/imagenes/descargando.gif"; }

    // ====== Utilidades de estilo ======

    protected TitledBorder bordeTitulado(String titulo) {
        TitledBorder b = BorderFactory.createTitledBorder(BorderFactory.createLineBorder(colorBotonBase().darker(), 1), titulo);
        b.setTitleColor(colorTextoBase());
        return b;
    }

    protected void estilizarBoton(JButton btn) { estilizarBoton(btn, 10); }
    protected void estilizarBoton(JButton btn, int paddingV) {
        if (!CrashDetectorGUI.esMac()) {
            btn.setBackground(colorBotonBase());
            btn.setForeground(colorTextoBase());
    ...
    protected void estilizarCombo(JComboBox<?> combo) {
        if (!CrashDetectorGUI.esMac()) {
            combo.setBackground(colorBotonBase());
            combo.setForeground(colorTextoBase());
        }
        combo.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        combo.setPreferredSize(new Dimension(220, 32));
    }

    protected Font negrita(Font base, float size) {
        return base.deriveFont(Font.BOLD, size);
    }

    /** Envuelve HTML sin escapar, aplicando colores/estilo del proyecto. */
    protected String htmlWrap(String innerHtml) {
        String fg = rgb(colorTextoBase());
        String bg = rgb(colorCajaTextoBase());
    ...
    protected String rgb(Color c) {
        return String.format("#%02x%02x%02x", c.getRed(), c.getGreen(), c.getBlue());
    }

    // Utilidades de imágenes

    protected ImageIcon cargarGif(String ruta) {
        try {
    ...
    protected ImageIcon cargarIconoAyudaFlexible(String ruta) {
        URL url = getClass().getResource(ruta);
        if (url == null) {
    ...
    protected ImageIcon cargarIconoEncajado(String ruta, int maxAncho, int maxAlto, boolean noAmpliar) {
    ...
    public void recargarApariencia() {
        aplicarApariencia(); // restablece colores y textos NO localizados
    }
    ...
    // La implementación concreta definirá `id()` y `init()` si necesita comportamiento adicional.
}
