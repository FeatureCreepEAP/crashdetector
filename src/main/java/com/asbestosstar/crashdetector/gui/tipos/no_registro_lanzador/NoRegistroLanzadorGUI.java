 ...
 *
 * - preparar(owner, instant): construye la UI, aplica apariencia y ata el
 * cierre de ventana a Omitir. - init(): visible(true) - Subclase define paleta
 * y aplica estilo en aplicarApariencia().
 */
public abstract class NoRegistroLanzadorGUI extends JDialog implements CrashDetectorGUI {
	...
	// **** Paneles estructurales promovidos a campos (para aplicar tema
	// correctamente) ****
	...
	protected String overlayMensaje() {
		return "Descargando y preparando enlaces...";
	}

	protected String gifDescargaPath() {
	...
	protected TitledBorder bordeTitulado(String titulo) {
		TitledBorder b = BorderFactory.createTitledBorder(BorderFactory.createLineBorder(colorBotonBase().darker(), 1),
				titulo);
	...
	// La implementación concreta definirá `id()` y `init()` si necesita
