 ...
 * - No referencia java.lang.Module directamente
 * - Añade dinámicamente:
 *      1) addReads: módulos externos leen crashdetector
 *      2) addOpens: crashdetector abre sus paquetes
 * - Registra transformer auxiliar
 * - Nunca interrumpe el arranque
 */
public class LibreMods {
	...
	public static void librar(Instrumentation inst) {
		...
		try {
			...
			try {
				inst.addTransformer(
						new DetectorDeModuloTransformer(),
						true
				);
			...
			Object moduloAgente =
					mGetModule.invoke(LibreMods.class);

			/* Instrumentation#getAllModules() */
			Method mGetAllModules =
					Instrumentation.class.getMethod("getAllModules");

			@SuppressWarnings("unchecked")
			Set<Object> todosLosModulos =
					(Set<Object>) mGetAllModules.invoke(inst);

			/* Module#getPackages() */
			Method mGetPackages =
					claseModule.getMethod("getPackages");

			@SuppressWarnings("unchecked")
			Set<String> paquetesAgente =
					(Set<String>) mGetPackages.invoke(moduloAgente);

			/* Instrumentation#redefineModule(...) */
			Method mRedefineModule =
			 ...
			 * 1) addReads
			 *
			 * Cada módulo externo debe leer crashdetector
			 * (dirección correcta para evitar IllegalAccessError)
			 * ===================================================== */
			for (Object moduloDestino : todosLosModulos) {
				...
				try {
					...
					Set<Object> extraReads =
							Collections.singleton(moduloAgente);

					mRedefineModule.invoke(
							inst,
							moduloDestino,                 // redefinir módulo externo
	...
	/**
	 * Transformer auxiliar de diagnóstico.
	 * No modifica bytecode.
	 */
	private static final class DetectorDeModuloTransformer
		...
		public byte[] transform(
			...
			try {
				...
				Object modulo =
						mGetModule.invoke(classBeingRedefined);
				...
				Method mGetName =
						modulo.getClass().getMethod("getName");

				String nombreModulo =
						(String) mGetName.invoke(modulo);
