...
public class ValidadorGPU {
	...
	private static String obtenerGPUActivaOpenGL() {
		...
		try {
			...
			if (nosotrosIniciamos) {
				try {
					if (ventanaCreada != 0)
						glfwDestroyWindow(ventanaCreada);
	...
	private static class InfoGPU {
		...
		public String toString() {
			...
			if (memoriaMB > 0)
				sb.append(" | VRAM: ").append(memoriaMB).append(" MB");
			if (!"N/A".equals(relojNucleo))
				sb.append(" | Núcleo: ").append(relojNucleo);
			if (!"N/A".equals(relojMemoria))
				sb.append(" | Mem: ").append(relojMemoria);
	...
	private static List<InfoGPU> obtenerGPUsDelSistema() {
		...
		// Sistemas Unix-like
		if (so.contains("nix") || so.contains("nbsd") || so.contains("bsd") || so.contains("sunos")
				|| so.contains("unix")) {
				...
				while ((linea = reader.readLine()) != null) {
					...
					if (lower.contains("vendor") || lower.contains("device")) {
						if (lower.contains("nvidia") || lower.contains("amd") || lower.contains("ati")
								...
								|| lower.contains("moore threads") || lower.contains("jingjia")
								|| lower.contains("zhaoxin")) {
							InfoGPU info = new InfoGPU();
...
// ==========================================================
	private static void clasificarGPU(InfoGPU info) {
		...
		if (lower.contains("matrox")) {
			boolean esDedicada = lower.contains("parhelia") || lower.contains("millennium") || lower.contains("m912")
	...
	private static boolean estaUsandoMejorGPU(String gpuActiva, List<InfoGPU> todasLasGPUs) {
		...
		if (infoActiva.tipo.equals("Acelerador Gráfico") || infoActiva.tipo.equals("Discreta (Alto Rendimiento)")
				|| infoActiva.tipo.equals("Integrada (Alto Rendimiento)")
				|| infoActiva.tipo.equals("Discreta (Nueva Generación)")) {
			return true;
		}

		// Si es Matrox Dedicada, se considera "Buena", pero verificamos si hay algo
		// MEJOR
		if (infoActiva.tipo.equals("Matrox_Dedicada")) {
			for (InfoGPU hw : todasLasGPUs) {
				if (hw.tipo.equals("Acelerador Gráfico") || hw.tipo.equals("Discreta (Alto Rendimiento)")) {
					CrashDetectorLogger
							.log("Detectada Matrox Dedicada activa, pero existe hardware superior: " + hw.nombre);
					return false;
				}
			}
			return true;
		...
		if (infoActiva.tipo.equals("Matrox_Embebida") || infoActiva.tipo.equals("Integrada")
				|| activaLower.contains("llvmpipe") || activaLower.contains("software")) {
			for (InfoGPU hw : todasLasGPUs) {
				if (hw.tipo.equals("Acelerador Gráfico") || hw.tipo.equals("Discreta (Alto Rendimiento)")
						|| hw.tipo.equals("Discreta (Nueva Generación)") || hw.tipo.equals("Matrox_Dedicada")) {
					CrashDetectorLogger
							.log("Detectada GPU de bajo rendimiento activa, pero se encontró hardware superior: "
									+ hw.nombre);
					return false;
				}
			}
