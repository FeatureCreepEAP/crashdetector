...
/**
 * Analiza errores cuando hay discrepancia entre el ID del mod en la anotación @Mod y el archivo mods.toml.
 * Detecta específicamente el error "The Mod File [...] has mods that were not found" que ocurre durante el desarrollo.
 */
public class ErrorDiscrepanciaModID implements Verificaciones {
    ...
    public void verificar(Consola consola) {
        ...
        // Analiza cada línea del registro buscando el patrón específico de error de discrepancia de IDs
        for (String linea : contenidoConsola.split(Verificaciones.nl)) {
            ...
            if (linea.contains("has mods that were not found")) {
                ...
                if (matcher.find()) {
                    ...
                    nombreMod = rutaMod.contains("\\") ? 
                        rutaMod.substring(rutaMod.lastIndexOf("\\") + 1) : 
                        rutaMod.contains("/") ? 
                        rutaMod.substring(rutaMod.lastIndexOf("/") + 1) : 
                        rutaMod;
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(nombre())
            .agregarEtiqueta(MonitorDePID.idioma.paso1_discrepancia_mod_id(nombreMod))
            .agregarEtiqueta(MonitorDePID.idioma.paso2_discrepancia_mod_id())
            .agregarEtiqueta(MonitorDePID.idioma.paso3_discrepancia_mod_id())
            .construir();
    }
}