...
/**
 * Detecta un conflicto crítico entre los mods Moonlight e Iceberg durante la recarga
 * de recursos, que provoca un fallo de OpenGL: "No context is current".
 * 
 * Este error ocurre cuando ambos mods intentan registrar listeners de recarga
 * de forma incompatible, especialmente en entornos Forge con mods de Fabric adaptados.
 */
public class ConflictoMoonlightIceberg implements Verificaciones {
    ...
    public boolean ocupaTrazo(TraceInfo trazo) {
        ...
        return t.contains("FATAL ERROR in native method")
            && t.contains("No context is current")
            && t.contains("moonlight")
            && t.contains("iceberg");
    }
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(nombre())
            .agregarEtiqueta(MonitorDePID.idioma.solucionConflictoMoonlightIceberg())
            .construir();
    }
