...
/**
 * Detecta problemas con el mod Supplementaries que impiden la carga del servidor
 * debido a errores en el registro de comportamientos de fuego, causando un
 * NullPointerException durante la carga de datapacks.
 */
public class ErrorSupplementariesCargaServidor implements Verificaciones {
    ...
    /**
     * Método de compatibilidad — busca si Supplementaries está presente en el contenido completo del registro.
     */
    @Override
    public void verificar(Consola consola) {
        // Verificamos si Supplementaries está presente en el contenido del registro
        if (consola.contenido_verificar != null) {
            encontradoSupplementaries = consola.contenido_verificar.toLowerCase().contains("net.mehvahdjukaar.supplementaries.common.block.fire_behaviors.firebehaviorsmanager.registerbehaviors");
        }
    }
    ...
    public void verificar(Consola consola, String linea, int numero_de_linea) {
        ...
        // Buscamos la combinación de mensajes que indican el problema de Supplementaries
        if (linea.contains("Failed to load datapacks")
        		&& encontradoSupplementaries) {
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(nombre())
                .agregarEtiqueta(MonitorDePID.idioma.pasoErrorSupplementariesCargaServidor())
                .construir();
    }
    ...
    public boolean ocupaTrazo(TraceInfo trazo) {
        ...
        return t.contains("Failed to load datapacks, can't proceed with server load")
                && t.contains("java.util.concurrent.ExecutionException")
                && t.contains("java.lang.NullPointerException")
                && t.contains("Cannot invoke")
                && t.contains("et.mehvahdjukaar.supplementaries.common.block.fire_behaviors.FireBehaviorsManager.registerBehaviors");
    }
