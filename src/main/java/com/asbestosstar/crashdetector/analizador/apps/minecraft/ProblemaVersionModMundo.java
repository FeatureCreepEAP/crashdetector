...
public class ProblemaVersionModMundo implements Verificaciones {
	...
	/**
	 * Verifica si el log contiene discrepancias de versión de mods en mundos
	 * guardados.
	 */
	@Override
	public void verificar(Consola consola) {
		...
		if (!nombresMods.isEmpty()) {
			if (nombresMods.size() > 1) {
				this.mensaje = MonitorDePID.idioma.mensajeVersionModMundoPlural(nombresMods, versionesEsperadas,
						versionesActuales);
			} else {
				this.mensaje = MonitorDePID.idioma.mensajeVersionModMundoSingular(nombresMods.get(0),
						versionesEsperadas.get(0), versionesActuales.get(0));
			}
			activado = true;
		}
	}
