...
/**
 * Detecta errores al cargar datos NBT de un mundo, típicamente causados por corrupción
 * del archivo level.dat, playerdata/*.dat o chunks.
 * Extrae el byte corrupto y proporciona orientación detallada para la reparación.
 */
public class ErrorCargaNBTMundoCorrupto implements Verificaciones {

    private boolean activado = false;
    private String mensaje = "";
    private static final Pattern PATRON_BYTE_CORRUPTO = Pattern.compile(
        "java\\.io\\.UTFDataFormatException: malformed input around byte (\\d+)"
    );
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(nombre())
            .agregarEtiqueta(MonitorDePID.idioma.solucionErrorCargaNBTMundoCorrupto())
            .construir();
    }
