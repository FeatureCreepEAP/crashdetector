...
/**
 * Detecta errores al crear modelos de bloques/items: 
 * "Failed to create model for <modid>:<nombre_modelo>".
 * 
 * Este error suele indicar que un mod tiene recursos corruptos, faltantes,
 * o incompatibles con la versión actual de Minecraft/Forge.
 */
public class ErrorCreacionModeloFallida implements Verificaciones {

    private boolean activado = false;
    private String mensaje = "";
    private static final Pattern PATRON_MODELO_FALLIDO = Pattern.compile(
        "java\\.lang\\.IllegalStateException: Failed to create model for ([^:\\s]+):([^\\s]+)"
    );
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(nombre())
            .agregarEtiqueta(MonitorDePID.idioma.solucionErrorCreacionModeloFallida())
            .construir();
    }
