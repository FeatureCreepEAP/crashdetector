 ...
 * - Archivo de animación faltante en el sistema de archivos del mod.
 * - Ruta mal configurada en el registro del modelo/animación.
...
public class AnimacionGeckoLibInExiste implements Verificaciones {
    ...
    public void verificar(Consola consola, String linea, int numero_de_linea) {
        ...
        if (linea.contains("GeckoLibException") && linea.contains("Unable to find animation file")) {
            ...
            // Intentar extraer el nombre del archivo antes de ": Unable to find animation file"
            int separador = linea.indexOf(": Unable to find animation file");
            if (separador > 0) {
                String temp = linea.substring(0, separador).trim();

                // La línea suele ser: "software.bernie.geckolib.GeckoLibException: modid:ruta/file.json"
