...
public class ConflictoOptiFineEMF implements Verificaciones {
     ...
     * <p>
     * Se busca el patrón característico del error de inyección crítica de EMF,
     * que ocurre cuando OptiFine y EMF no son compatibles.
     * </p>
     */
    @Override
    public void verificar(Consola consola, String linea, int numero_de_linea) {
        ...
        if (linea.contains("org.spongepowered.asm.mixin.injection.throwables.InjectionError")
                && linea.contains("Critical injection failure")
                && linea.contains("entity_model_features.mixins.json")
                && linea.contains("MixinBlockEntityWithoutLevelRenderer")
                && linea.contains("emf$setRenderFactory")) {
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(nombre())
                .agregarEtiqueta(MonitorDePID.idioma.pasoConflictoOptiFineEMF())
                .construir();
    }
    ...
    public boolean ocupaTrazo(TraceInfo trazo) {
        ...
        return t.contains("org.spongepowered.asm.mixin.injection.throwables.InjectionError")
                && t.contains("Critical injection failure")
                && t.contains("entity_model_features.mixins.json")
                && t.contains("MixinBlockEntityWithoutLevelRenderer")
                && t.contains("emf$setRenderFactory")
                && encontradoOptiFine;
    }
}