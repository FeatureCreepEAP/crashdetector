...
/**
 * Detecta errores de ServiceConfigurationError relacionados con ITransformationService
 * de ModLauncher, donde un proveedor no puede ser instanciado, lo que indica
 * un mod con una implementación defectuosa.
 */
public class ErrorModLauncherTransformationService implements Verificaciones {
    ...
    public void verificar(Consola consola, String linea, int numero_de_linea) {
        ...
        // Buscamos la línea que contiene el error de ServiceConfigurationError con ITransformationService
        if (linea.contains("java.util.ServiceConfigurationError")
                && linea.contains("cpw.mods.modlauncher.api.ITransformationService")
                && linea.contains("Provider") 
            ...
            if (inicioClase > -1 && finClase > inicioClase) {
                ...
                // Si no encontramos el patrón exacto, intentamos encontrar la clase de otra manera
            ...
            mensaje = MonitorDePID.idioma.errorModLauncherTransformationService(claseProveedor) + Verificaciones.nl_html;
            activado = true;
        }
    }
    ...
    public boolean ocupaTrazo(TraceInfo trazo) {
        ...
        return t.contains("java.util.ServiceConfigurationError")
                && t.contains("cpw.mods.modlauncher.api.ITransformationService")
                && t.contains("Provider")
