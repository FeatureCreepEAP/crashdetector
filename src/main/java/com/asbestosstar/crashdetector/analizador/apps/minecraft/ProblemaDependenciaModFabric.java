...
public class ProblemaDependenciaModFabric implements Verificaciones {
	...
	private void procesarLineaError(String linea) {
		// Ejemplo: - Mod 'Indium' (indium) 1.0.27+mc1.20.1 requires version 0.5.3 of
		// mod 'Sodium' (sodium), but only the wrong version is present:
		// 0.4.10+build.27!
		// Ejemplo: - Mod 'Indium' (indium) 1.0.27+mc1.20.1 requires version 3.2.0 or
		// later of fabric-renderer-api-v1, which is missing!
			...
			if (linea.contains("requires version")) {
				...
				if (linea.contains("mod '")) {
					...
					if (!version.isEmpty() && linea.contains("present:")) {
						...
						if (inicioActual > 0) {
							String versionActual = linea
									.substring(inicioActual, (finActual > 0) ? finActual : linea.length()).trim();
