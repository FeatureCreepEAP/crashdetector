...
/**
 * Clase que detecta mods incompatibles con la versión de Minecraft en Forge.Gracias a Aternos por que esta es una implementacion de su codex https://github.com/aternosorg/codex-minecraft
 */
public class ProblemaModIncompatibleConMinecraft implements Verificaciones {
    ...
    public void verificar(Consola consola) {
        ...
        // Patrón para detectar mods que no son compatibles con la versión actual de Minecraft
        ...
        if (!nombresMods.isEmpty()) {
            if (nombresMods.size() > 1) {
                this.mensaje = MonitorDePID.idioma.mensajeModIncompatibleConMinecraftPlural(nombresMods, versionesMinecraft);
            } else {
                this.mensaje = MonitorDePID.idioma.mensajeModIncompatibleConMinecraftSingular(nombresMods.get(0), versionesMinecraft.get(0));
            }
            activado = true;
        }
    }
