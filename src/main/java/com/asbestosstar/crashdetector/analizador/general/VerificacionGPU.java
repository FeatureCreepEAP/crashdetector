...
public class VerificacionGPU implements Verificaciones {
    ...
    public void verificar(Consola consola) {
        ...
        if ((inicio && !fin) || (openglInicio && !openglFin)) {
            ...
            this.mensaje = MonitorDePID.idioma.gpu_crash_posible()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_crash_causas()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_crash_recomendaciones()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_nota_precision()
                    + Verificaciones.nl_html
        ...
        if (advertenciaGPU) {
            ...
            this.mensaje = MonitorDePID.idioma.gpu_no_optima()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_no_optima_detalles()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_nota_precision()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_consumo_energia()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_recomendaciones_rendimiento()
                    + Verificaciones.nl_html
                    + MonitorDePID.idioma.gpu_parche_info();
        }
    }
    ...
    public QuickFix solucion() {
        return new QuickFix.Builder(MonitorDePID.idioma.nombre_verificacion_gpu())
                .agregarBoton(MonitorDePID.idioma.desactivar_parche_gpu(), retener -> {
                    ConfigDeParches.obtenerInstancia().establecerActivo("transformacion_de_minecraft_codigo_gpu", false);
                }, true)
                .construir();
    }
